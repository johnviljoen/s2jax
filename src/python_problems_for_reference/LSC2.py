...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        if('X' in ix_):
            ...
            self.y0  = (
                  arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X']),float(98.0)))
        if('Y' in ix_):
            self.x0[ix_['Y']] = float(36.0)
        else:
            self.y0  = (
                  arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['Y']),float(36.0)))
        if('R' in ix_):
            self.x0[ix_['R']] = float(270.0)
        else:
            self.y0  = (
                  arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['R']),float(270.0)))
        pass
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
