...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        if('B1' in ix_):
            ...
            self.y0  = (
                  arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['B1']),float(10.0)))
        if('B2' in ix_):
        ...
        if('B3' in ix_):
            ...
            self.y0  = (
                  arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['B3']),float(0.05)))
        if('B4' in ix_):
            self.x0[ix_['B4']] = float(-0.00001)
        else:
            self.y0  = (
                  arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['B4']),float(-0.00001)))
        if('B5' in ix_):
            self.x0[ix_['B5']] = float(-0.05)
        else:
            self.y0  = (
                  arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['B5']),float(-0.05)))
        if('B6' in ix_):
            self.x0[ix_['B6']] = float(0.001)
        else:
            self.y0  = (
                  arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['B6']),float(0.001)))
        if('B7' in ix_):
            self.x0[ix_['B7']] = float(-0.000001)
        else:
            self.y0  = (
                  arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['B7']),float(-0.000001)))
        pass
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
