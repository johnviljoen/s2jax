...
# IE NPTS                5000           $-PARAMETER n = 10003
        ...
        for I in range(int(v_['TDPulo']),int(v_['TDPuhi'])+1):
            if('X'+str(I) in ix_):
                self.x0[ix_['X'+str(I)]] = float(1.8)
            else:
                self.y0  = (
                      arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X'+str(I)]),float(1.8)))
            if('Y'+str(I) in ix_):
                self.x0[ix_['Y'+str(I)]] = float(1.0)
            else:
                self.y0  = (
                      arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['Y'+str(I)]),float(1.0)))
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
