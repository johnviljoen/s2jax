...
# **********************
    ...
    def eEXPT(self, nargout,*args):
        ...
        EV_  = args[0]
        iel_ = args[1]
        EXPA = np.exp(self.elpar[iel_][0]*EV_[0])
        f_   = EXPA
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = np.zeros(dim)
            g_[0] = self.elpar[iel_][0]*EXPA
            if nargout>2:
                H_ = np.zeros((1,1))
                H_[0,0] = self.elpar[iel_][0]*self.elpar[iel_][0]*EXPA
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
