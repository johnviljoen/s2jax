...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        for I in range(int(v_['0']),int(v_['M'])+1):
            ...
            v_['C'] = v_['2S4T']-v_['E2T']
            self.gconst = arrset(self.gconst,ig_['CG'+str(I)],float(v_['C']))
        #%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        for I in range(int(v_['0']),int(v_['M'])+1):
            ename = 'E'+str(I)
            [ie,ie_,_] = s2mpj_ii(ename,ie_)
            self.elftype = arrset(self.elftype,ie,'eXEYT')
            ...
            v_['RI'] = float(I)
            v_['T'] = v_['RI']*v_['H']
            posep = np.where(elftp[ielftype[ie]]=='TT')[0]
...
# **********************
    ...
    def eXEYT(self, nargout,*args):
        ...
        EV_  = args[0]
        iel_ = args[1]
        EYT = np.exp(EV_[1]*self.elpar[iel_][0])
        f_   = EV_[0]*EYT
        if not isinstance( f_, float ):
            f_   = f_.item();
        if nargout>1:
            ...
            g_ = np.zeros(dim)
            g_[0] = EYT
            g_[1] = EV_[0]*self.elpar[iel_][0]*EYT
            if nargout>2:
                H_ = np.zeros((2,2))
                H_[0,1] = self.elpar[iel_][0]*EYT
                H_[1,0] = H_[0,1]
                H_[1,1] = EV_[0]*self.elpar[iel_][0]*self.elpar[iel_][0]*EYT
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
