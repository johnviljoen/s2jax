...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        self.gconst = jtu.arrset(self.gconst,ig_['F'],float(-1.0))
        self.gconst = jtu.arrset(self.gconst,ig_['CON1'],float(-0.0401))
        self.gconst = jtu.arrset(self.gconst,ig_['CON2'],float(-0.010085))
        #%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        self.xlower = jnp.full((self.n,1),0.001)
        self.xupper = jnp.full((self.n,1),+float('inf'))
        self.xupper = jtu.np_like_set(self.xupper, ix_['X1'], 4.0e5)
        self.xupper = jtu.np_like_set(self.xupper, ix_['X2'], 3.0e5)
        self.xupper = jtu.np_like_set(self.xupper, ix_['X3'], 2.0e5)
        self.xupper = jtu.np_like_set(self.xupper, ix_['X4'], 1.0e5)
        #%%%%%%%%%%%%%%%%%% START POINT %%%%%%%%%%%%%%%%%%
        self.x0 = jnp.full((self.n,1),float(1.0))
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
        [it,iet_,_] = jtu.s2mpj_ii( 'en1dX', iet_)
        ...
        self.elftype = jtu.arrset(self.elftype,ie,'en1dX')
        ielftype = jtu.arrset(ielftype,ie,iet_["en1dX"])
        vname = 'X1'
        [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,float(0.001),None,float(1.0))
        ...
        self.grelw = jtu.loaset(self.grelw,ig,posel,float(-2.25))
        posel = len(self.grelt[ig])
        ...
        self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['E1'])
        nlc = jnp.union1d(nlc,jnp.array([ig]))
        self.grelw = jtu.loaset(self.grelw,ig,posel,float(-0.16))
        posel = posel+1
        self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['E2'])
        self.grelw = jtu.loaset(self.grelw,ig,posel,float(-0.36))
        posel = len(self.grelt[ig])
        self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['E3'])
        nlc = jnp.union1d(nlc,jnp.array([ig]))
        self.grelw = jtu.loaset(self.grelw,ig,posel,float(-0.64))
        posel = posel+1
        self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['E4'])
        self.grelw = jtu.loaset(self.grelw,ig,posel,float(-0.64))
        #%%%%%%%%%%%%%%%%%% OBJECT BOUNDS %%%%%%%%%%%%%%%%%
