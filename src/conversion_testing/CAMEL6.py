...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        self.xupper = jtu.np_like_set(self.xupper, ix_['X1'], 3.0)
        self.xlower = jtu.np_like_set(self.xlower, ix_['X1'], -3.0)
        self.xupper = jtu.np_like_set(self.xupper, ix_['X2'], 1.5)
        self.xlower = jtu.np_like_set(self.xlower, ix_['X2'], -1.5)
        #%%%%%%%%%%%%%%%%%%% START POINT %%%%%%%%%%%%%%%%%%
        self.x0 = jnp.zeros((self.n,1))
        self.x0 = jtu.np_like_set(self.x0, ix_['X1'], float(1.1))
        self.x0 = jtu.np_like_set(self.x0, ix_['X2'], float(1.1))
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
        ...
        [it,iet_,_] = jtu.s2mpj_ii( 'eSIXPW', iet_)
        elftv = jtu.loaset(elftv,it,0,'V1')
        [it,iet_,_] = jtu.s2mpj_ii( 'eFORPW', iet_)
        elftv = jtu.loaset(elftv,it,0,'V1')
        #%%%%%%%%%%%%%%%%%% ELEMENT USES %%%%%%%%%%%%%%%%%%
        ie_ = {}
        ...
        ename = 'E2'
        [ie,ie_,_] = jtu.s2mpj_ii(ename,ie_)
        self.elftype = jtu.arrset(self.elftype,ie,'eFORPW')
        ielftype = jtu.arrset(ielftype,ie,iet_["eFORPW"])
        vname = 'X1'
        [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,None,None,None)
        posev = jnp.where(elftv[ielftype[ie]]=='V1')[0]
        self.elvar = jtu.loaset(self.elvar,ie,posev[0],iv)
        ename = 'E3'
        [ie,ie_,_] = jtu.s2mpj_ii(ename,ie_)
        self.elftype = jtu.arrset(self.elftype,ie,'eSIXPW')
        ielftype = jtu.arrset(ielftype,ie,iet_["eSIXPW"])
        vname = 'X1'
        [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,None,None,None)
        posev = jnp.where(elftv[ielftype[ie]]=='V1')[0]
        self.elvar = jtu.loaset(self.elvar,ie,posev[0],iv)
        ...
        self.grelw = jtu.loaset(self.grelw,ig,posel,float(-2.1))
        posel = len(self.grelt[ig])
        self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['E3'])
        self.grelw = jtu.loaset(self.grelw,ig,posel,float(0.3333333333))
        posel = posel+1
    ...
    def eFORPW(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, 4.0*EV_[0]**3.0)
            if nargout>2:
                H_ = jnp.zeros((1,1))
    ...
    def eSIXPW(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, 6.0*EV_[0]**5.0)
            if nargout>2:
                H_ = jnp.zeros((1,1))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), 30.0*EV_[0]**4.0)
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
