...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        for I in range(int(v_['0']),int(v_['M'])+1):
            ...
            v_['2T'] = v_['T']+v_['T']
            v_['4T'] = v_['2T']+v_['2T']
            v_['E2T'] = jnp.exp(v_['2T'])
            v_['S4T'] = jnp.sin(v_['4T'])
            v_['2S4T'] = v_['S4T']+v_['S4T']
            v_['C'] = v_['2S4T']-v_['E2T']
            self.gconst = jtu.arrset(self.gconst,ig_['CG'+str(I)],float(v_['C']))
        #%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        [it,iet_,_] = jtu.s2mpj_ii( 'eXEYT', iet_)
        elftv = jtu.loaset(elftv,it,0,'X')
        elftv = jtu.loaset(elftv,it,1,'Y')
        elftp = []
        elftp = jtu.loaset(elftp,it,0,'TT')
        #%%%%%%%%%%%%%%%%%% ELEMENT USES %%%%%%%%%%%%%%%%%%
        ie_ = {}
        ...
        for I in range(int(v_['0']),int(v_['M'])+1):
            ename = 'E'+str(I)
            [ie,ie_,_] = jtu.s2mpj_ii(ename,ie_)
            self.elftype = jtu.arrset(self.elftype,ie,'eXEYT')
            ielftype = jtu.arrset(ielftype,ie,iet_["eXEYT"])
            vname = 'X2'
            [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,None,None,None)
            posev = jnp.where(elftv[ielftype[ie]]=='X')[0]
            self.elvar = jtu.loaset(self.elvar,ie,posev[0],iv)
            ...
            v_['RI'] = float(I)
            v_['T'] = v_['RI']*v_['H']
            posep = jnp.where(elftp[ielftype[ie]]=='TT')[0]
...
# **********************
    ...
    @staticmethod
    def eXEYT(self, nargout,*args):

        import jax.numpy as jnp
        EV_  = args[0]
        iel_ = args[1]
        EYT = jnp.exp(EV_[1]*self.elpar[iel_][0])
        f_   = EV_[0]*EYT
        if not isinstance( f_, float ):
            f_   = f_.item();
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, EYT)
            g_ = jtu.np_like_set(g_, 1, EV_[0]*self.elpar[iel_][0]*EYT)
            if nargout>2:
                H_ = jnp.zeros((2,2))
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), self.elpar[iel_][0]*EYT)
                H_ = jtu.np_like_set(H_, jnp.array([1,0]), H_[0,1])
                H_ = jtu.np_like_set(H_, jnp.array([1,1]), EV_[0]*self.elpar[iel_][0]*self.elpar[iel_][0]*EYT)
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
