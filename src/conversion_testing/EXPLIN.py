    ...
    def eEXP(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, 0.1*EV_[1]*F)
            g_ = jtu.np_like_set(g_, 1, 0.1*EV_[0]*F)
            if nargout>2:
                H_ = jnp.zeros((2,2))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), (0.1*EV_[1])**2*F)
                H_ = jtu.np_like_set(H_, jnp.array([1,1]), (0.1*EV_[0])**2*F)
                H_ = jtu.np_like_set(H_, jnp.array([1,0]), (0.1+0.01*EV_[0]*EV_[1])*F)
