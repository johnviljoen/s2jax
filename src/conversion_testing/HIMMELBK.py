...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        valA = jtu.append(valA,float(0.0693))
        irA  = jtu.append(irA,[ig])
        icA  = jtu.append(icA,[ix_['X2']])
        valA = jtu.append(valA,float(0.0577))
        irA  = jtu.append(irA,[ig])
        ...
        valA = jtu.append(valA,float(0.26))
        irA  = jtu.append(irA,[ig])
        icA  = jtu.append(icA,[ix_['X6']])
        valA = jtu.append(valA,float(0.55))
        irA  = jtu.append(irA,[ig])
        ...
        valA = jtu.append(valA,float(0.12))
        irA  = jtu.append(irA,[ig])
        icA  = jtu.append(icA,[ix_['X10']])
        valA = jtu.append(valA,float(0.18))
        irA  = jtu.append(irA,[ig])
        ...
        for I in range(int(v_['1']),int(v_['12'])+1):
            [ig,ig_,_] = jtu.s2mpj_ii('CA'+str(I),ig_)
            gtype = jtu.arrset(gtype,ig,'==')
            cnames = jtu.arrset(cnames,ig,'CA'+str(I))
        for I in range(int(v_['1']),int(v_['24'])+1):
            [ig,ig_,_] = jtu.s2mpj_ii('CA13',ig_)
            gtype = jtu.arrset(gtype,ig,'==')
            cnames = jtu.arrset(cnames,ig,'CA13')
            irA  = jtu.append(irA,[ig])
        ...
        for I in range(int(v_['1']),int(v_['12'])+1):
            ...
            [ig,ig_,_] = jtu.s2mpj_ii('CA14',ig_)
            gtype = jtu.arrset(gtype,ig,'==')
            cnames = jtu.arrset(cnames,ig,'CA14')
            irA  = jtu.append(irA,[ig])
            icA  = jtu.append(icA,[ix_['X'+str(I)]])
            valA = jtu.append(valA,float(v_['1/DI']))
            v_['F/BI+12'] = v_['F']/v_['B'+str(int(v_['I+12']))]
            irA  = jtu.append(irA,[ig])
            icA  = jtu.append(icA,[ix_['X'+str(int(v_['I+12']))]])
            valA = jtu.append(valA,float(v_['F/BI+12']))
        #%%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
        self.n   = len(ix_)
        ngrp   = len(ig_)
        ...
        self.gconst = jtu.arrset(self.gconst,ig_['CA13'],float(1.0))
        self.gconst = jtu.arrset(self.gconst,ig_['CA14'],float(1.671))
        #%%%%%%%%%%%%%%%%%% START POINT %%%%%%%%%%%%%%%%%%
        self.x0 = jnp.full((self.n,1),float(0.04))
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
        ...
        for I in range(int(v_['1']),int(v_['12'])+1):
            ...
            for J in range(int(v_['1']),int(v_['12'])+1):
                ...
                [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,None,None,float(0.04))
        ...
        for I in range(int(v_['1']),int(v_['12'])+1):
            ...
            for J in range(int(v_['1']),int(v_['12'])+1):
                ...
                self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['40BI/BJ']))
            for J in range(int(v_['13']),int(v_['24'])+1):
                ...
                self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['E'+str(I)+','+str(J)])
                nlc = jnp.union1d(nlc,jnp.array([ig]))
                self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['-CB+/BJ']))
        #%%%%%%%%%%%%%%%%%% OBJECT BOUNDS %%%%%%%%%%%%%%%%%
#    Solution
# LO SOLTN                0.0893344
