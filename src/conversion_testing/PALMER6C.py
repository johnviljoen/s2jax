...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        [iv,ix_,_] = jtu.s2mpj_ii('A12',ix_)
        self.xnames=jtu.arrset(self.xnames,iv,'A12')
        [iv,ix_,_] = jtu.s2mpj_ii('A14',ix_)
        self.xnames=jtu.arrset(self.xnames,iv,'A14')
        #%%%%%%%%%%%%%%%%%%  DATA GROUPS %%%%%%%%%%%%%%%%%%%
        ...
        for I in range(int(v_['12']),int(v_['M'])+1):
            ...
            valA = jtu.append(valA,float(v_['X**6']))
            irA  = jtu.append(irA,[ig])
            icA  = jtu.append(icA,[ix_['A8']])
            valA = jtu.append(valA,float(v_['X**8']))
            irA  = jtu.append(irA,[ig])
            icA  = jtu.append(icA,[ix_['A10']])
            valA = jtu.append(valA,float(v_['X**10']))
            irA  = jtu.append(irA,[ig])
            icA  = jtu.append(icA,[ix_['A12']])
            valA = jtu.append(valA,float(v_['X**12']))
            irA  = jtu.append(irA,[ig])
            icA  = jtu.append(icA,[ix_['A14']])
            valA = jtu.append(valA,float(v_['X**14']))
        #%%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
        self.n   = len(ix_)
        ngrp   = len(ig_)
        ...
        self.xlower = jtu.np_like_set(self.xlower, ix_['A12'], -float('Inf'))
        self.xupper = jtu.np_like_set(self.xupper, ix_['A12'], +float('Inf'))
        self.xlower = jtu.np_like_set(self.xlower, ix_['A14'], -float('Inf'))
        self.xupper = jtu.np_like_set(self.xupper, ix_['A14'], +float('Inf'))
        #%%%%%%%%%%%%%%%%%% START POINT %%%%%%%%%%%%%%%%%%
