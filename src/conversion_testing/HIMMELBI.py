...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        gtype        = jnp.array([])
        for J in range(int(v_['1']),int(v_['NT'])+1):
            [ig,ig_,_] = jtu.s2mpj_ii('P'+str(J),ig_)
            gtype = jtu.arrset(gtype,ig,'<>')
            v_['1/UJ'] = 1.0/v_['U'+str(J)]
            self.gscale = jtu.arrset(self.gscale,ig,float(v_['1/UJ']))
        v_['J'] = 1
        for I in range(int(v_['1']),int(v_['5'])+1):
            [ig,ig_,_] = jtu.s2mpj_ii('CB'+str(int(v_['J'])),ig_)
            gtype = jtu.arrset(gtype,ig,'>=')
            cnames = jtu.arrset(cnames,ig,'CB'+str(int(v_['J'])))
        ...
        #%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        self.xupper = jnp.full((self.n,1),v_['NW'])
        self.xlower = jnp.zeros((self.n,1))
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
        [it,iet_,_] = jtu.s2mpj_ii( 'en5PEXP', iet_)
        ...
        elftp = jtu.loaset(elftp,it,2,'A3')
        elftp = jtu.loaset(elftp,it,3,'A4')
        elftp = jtu.loaset(elftp,it,4,'A5')
        #%%%%%%%%%%%%%%%%%% ELEMENT USES %%%%%%%%%%%%%%%%%%
        ie_ = {}
        ...
        for J in range(int(v_['1']),int(v_['NT'])+1):
            ...
            self.elftype = jtu.arrset(self.elftype,ie,'en5PEXP')
            ielftype = jtu.arrset(ielftype,ie,iet_["en5PEXP"])
            self.x0 = jnp.zeros((self.n,1))
            vname = 'X'+str(int(v_['1']))+','+str(J)
            [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,None,float(v_['NW']),None)
        ...
        for J in range(int(v_['1']),int(v_['NT'])+1):
            ...
            self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['PP'+str(J)])
...
# **********************
    ...
    def en5PEXP(self, nargout,*args):
        ...
        EV_  = args[0]
        iel_ = args[1]
        LA1 = jnp.log(self.elpar[iel_][0])
        LA2 = jnp.log(self.elpar[iel_][1])
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, LA1*FF)
            g_ = jtu.np_like_set(g_, 1, LA2*FF)
            g_ = jtu.np_like_set(g_, 2, LA3*FF)
            g_ = jtu.np_like_set(g_, 3, LA4*FF)
            g_ = jtu.np_like_set(g_, 4, LA5*FF)
            if nargout>2:
                H_ = jnp.zeros((5,5))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), LA1*LA1*FF)
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), LA1*LA2*FF)
                H_ = jtu.np_like_set(H_, jnp.array([1,0]), H_[0,1])
                H_ = jtu.np_like_set(H_, jnp.array([0,2]), LA1*LA3*FF)
                H_ = jtu.np_like_set(H_, jnp.array([2,0]), H_[0,2])
                H_ = jtu.np_like_set(H_, jnp.array([0,3]), LA1*LA4*FF)
                H_ = jtu.np_like_set(H_, jnp.array([3,0]), H_[0,3])
                H_ = jtu.np_like_set(H_, jnp.array([0,4]), LA1*LA5*FF)
                H_ = jtu.np_like_set(H_, jnp.array([4,0]), H_[0,4])
                H_ = jtu.np_like_set(H_, jnp.array([1,1]), LA2*LA2*FF)
                H_ = jtu.np_like_set(H_, jnp.array([1,2]), LA2*LA3*FF)
                H_ = jtu.np_like_set(H_, jnp.array([2,1]), H_[1,2])
                H_ = jtu.np_like_set(H_, jnp.array([1,3]), LA2*LA4*FF)
                H_ = jtu.np_like_set(H_, jnp.array([3,1]), H_[1,3])
                H_ = jtu.np_like_set(H_, jnp.array([1,4]), LA2*LA5*FF)
                H_ = jtu.np_like_set(H_, jnp.array([4,1]), H_[1,4])
                H_ = jtu.np_like_set(H_, jnp.array([2,2]), LA3*LA3*FF)
                H_ = jtu.np_like_set(H_, jnp.array([2,3]), LA3*LA4*FF)
                H_ = jtu.np_like_set(H_, jnp.array([3,2]), H_[2,3])
                H_ = jtu.np_like_set(H_, jnp.array([2,4]), LA3*LA5*FF)
                H_ = jtu.np_like_set(H_, jnp.array([4,2]), H_[2,4])
                H_ = jtu.np_like_set(H_, jnp.array([3,3]), LA4*LA4*FF)
                H_ = jtu.np_like_set(H_, jnp.array([3,4]), LA4*LA5*FF)
                H_ = jtu.np_like_set(H_, jnp.array([4,3]), H_[3,4])
                H_ = jtu.np_like_set(H_, jnp.array([4,4]), LA5*LA5*FF)
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
