...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        if('X0,2' in ix_):
            ...
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X0,2']),float(0.2)))
        if('X0,3' in ix_):
            self.x0 = jtu.np_like_set(self.x0, jnp.array([ix_['X0,3']]), float(0.9))
        else:
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X0,3']),float(0.9)))
        if('X1,1' in ix_):
        ...
        if('X1,2' in ix_):
            ...
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X1,2']),float(0.2)))
        if('X1,3' in ix_):
            self.x0 = jtu.np_like_set(self.x0, jnp.array([ix_['X1,3']]), float(0.8))
        else:
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X1,3']),float(0.8)))
        if('X2,1' in ix_):
            self.x0 = jtu.np_like_set(self.x0, jnp.array([ix_['X2,1']]), float(0.05))
        else:
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X2,1']),float(0.05)))
        if('X2,2' in ix_):
        ...
        if('X2,3' in ix_):
            ...
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X2,3']),float(0.8)))
        if('X3,1' in ix_):
            self.x0 = jtu.np_like_set(self.x0, jnp.array([ix_['X3,1']]), float(0.1))
        else:
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X3,1']),float(0.1)))
        if('X3,2' in ix_):
            self.x0 = jtu.np_like_set(self.x0, jnp.array([ix_['X3,2']]), float(0.3))
        else:
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X3,2']),float(0.3)))
        if('X3,3' in ix_):
            self.x0 = jtu.np_like_set(self.x0, jnp.array([ix_['X3,3']]), float(0.6))
        else:
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X3,3']),float(0.6)))
        if('X4,1' in ix_):
            self.x0 = jtu.np_like_set(self.x0, jnp.array([ix_['X4,1']]), float(0.3))
        else:
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X4,1']),float(0.3)))
        if('X4,2' in ix_):
            self.x0 = jtu.np_like_set(self.x0, jnp.array([ix_['X4,2']]), float(0.5))
        else:
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X4,2']),float(0.5)))
        if('X4,3' in ix_):
            self.x0 = jtu.np_like_set(self.x0, jnp.array([ix_['X4,3']]), float(0.3))
        else:
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X4,3']),float(0.3)))
        if('X5,1' in ix_):
            self.x0 = jtu.np_like_set(self.x0, jnp.array([ix_['X5,1']]), float(0.6))
        else:
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X5,1']),float(0.6)))
        if('X5,2' in ix_):
            self.x0 = jtu.np_like_set(self.x0, jnp.array([ix_['X5,2']]), float(0.6))
        else:
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X5,2']),float(0.6)))
        if('X5,3' in ix_):
            self.x0 = jtu.np_like_set(self.x0, jnp.array([ix_['X5,3']]), float(0.0))
        else:
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X5,3']),float(0.0)))
        for I in range(int(v_['0']),int(v_['N-1'])+1):
            if('T'+str(I) in ix_):
