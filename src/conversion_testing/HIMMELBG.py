...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        iet_  = {}
        elftv = []
        [it,iet_,_] = jtu.s2mpj_ii( 'eHG', iet_)
        ...
        self.elftype = jtu.arrset(self.elftype,ie,'eHG')
        ielftype = jtu.arrset(ielftype,ie,iet_["eHG"])
        vname = 'X1'
    ...
    def eHG(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, EX*(DFCDX-FC))
            g_ = jtu.np_like_set(g_, 1, EX*(DFCDY-FC))
            if nargout>2:
                H_ = jnp.zeros((2,2))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), EX*(FC-2.0*DFCDX+4.0))
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), EX*(FC-DFCDY-DFCDX))
                H_ = jtu.np_like_set(H_, jnp.array([1,0]), H_[0,1])
                H_ = jtu.np_like_set(H_, jnp.array([1,1]), EX*(FC-2.0*DFCDY+6.0))
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
