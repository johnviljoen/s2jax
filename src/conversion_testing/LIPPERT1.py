...
# IE NY                  100            $-PARAMETER
        ...
        for I in range(int(v_['1']),int(v_['NX'])+1):
            ...
            for J in range(int(v_['1']),int(v_['NY'])+1):
                ...
                self.gscale = jtu.arrset(self.gscale,ig,float(v_['DX']))
                irA  = jtu.append(irA,[ig])
        ...
        for I in range(int(v_['1']),int(v_['NX'])+1):
            for J in range(int(v_['1']),int(v_['NY'])+1):
                ...
                self.gconst = jtu.arrset(self.gconst,ig_['C'+str(I)+','+str(J)],float(1.0))
                self.gconst = jtu.arrset(self.gconst,ig_['D'+str(I)+','+str(J)],float(1.0))
        #%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        self.xlower = jnp.full((self.n,1),-float('Inf'))
        self.xupper = jnp.full((self.n,1),+float('Inf'))
        self.xlower = jtu.np_like_set(self.xlower, ix_['T'], 0.01)
        #%%%%%%%%%%%%%%%%%%% START POINT %%%%%%%%%%%%%%%%%%
