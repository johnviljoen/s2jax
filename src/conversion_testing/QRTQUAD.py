...
# IE M                   1100           $-PARAMETER
        ...
        iet_  = {}
        elftv = []
        [it,iet_,_] = jtu.s2mpj_ii( 'eQUART', iet_)
        ...
        for I in range(int(v_['1']),int(v_['M'])+1):
            ...
            self.elftype = jtu.arrset(self.elftype,ie,'eQUART')
            ielftype = jtu.arrset(ielftype,ie,iet_["eQUART"])
            vname = 'X'+str(I)
    ...
    def eQUART(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, self.elpar[iel_][0]*EV_[1]*4.0*XY**3)
            g_ = jtu.np_like_set(g_, 1, self.elpar[iel_][0]*EV_[0]*4.0*XY**3)
            if nargout>2:
                H_ = jnp.zeros((2,2))
