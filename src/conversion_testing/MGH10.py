...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        if('B2' in ix_):
            ...
            self.y0  = (                   jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['B2']),float(400000.0)))
        if('B3' in ix_):
            self.x0 = jtu.np_like_set(self.x0, ix_['B3'], float(25000.0))
        else:
            self.y0  = (                   jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['B3']),float(25000.0)))
        pass
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
...
# **********************
    ...
    def eE12(self, nargout,*args):
        ...
        if nargout>1:
            ...
            g_ = jtu.np_like_set(g_, 1, F/V3PX)
            g_ = jtu.np_like_set(g_, 2, -F*EV_[1]/V3PX2)
            if nargout>2:
                H_ = jnp.zeros((3,3))
                ...
                H_ = jtu.np_like_set(H_, jnp.array([1,1]), F/V3PX2)
                H_ = jtu.np_like_set(H_, jnp.array([1,2]), -F*(1.0/V3PX2+EV_[1]/V3PX3))
                H_ = jtu.np_like_set(H_, jnp.array([2,1]), H_[1,2])
                H_ = jtu.np_like_set(H_, jnp.array([2,2]), F*EV_[1]*(EV_[1]/V3PX**4+2.0/V3PX3))
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
