...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        for I in range(int(v_['0']),int(v_['M'])+1):
            ...
            v_['-EXPW'] = -1.0*v_['EXPW']
            self.gconst = jtu.arrset(self.gconst,ig_['LO'+str(I)],float(v_['-EXPW']))
            self.gconst = jtu.arrset(self.gconst,ig_['UP'+str(I)],float(v_['EXPW']))
        #%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
    ...
    def eRATIO(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, 1.0/DENOM)
            g_ = jtu.np_like_set(g_, 1, self.elpar[iel_][0]/DENOM)
            g_ = jtu.np_like_set(g_, 2, -NUMER*self.elpar[iel_][0]/DENOM**2)
            if nargout>2:
                H_ = jnp.zeros((3,3))
                H_ = jtu.np_like_set(H_, jnp.array([0,2]), -self.elpar[iel_][0]/DENOM**2)
                H_ = jtu.np_like_set(H_, jnp.array([2,0]), H_[0,2])
                H_ = jtu.np_like_set(H_, jnp.array([1,2]), -self.elpar[iel_][0]*self.elpar[iel_][0]/DENOM**2)
                H_ = jtu.np_like_set(H_, jnp.array([2,1]), H_[1,2])
                H_ = jtu.np_like_set(H_, jnp.array([2,2]), 2.0*NUMER*self.elpar[iel_][0]*self.elpar[iel_][0]/DENOM**3)
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
