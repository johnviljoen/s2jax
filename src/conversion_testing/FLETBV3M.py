...
# RE KAPPA               0.0            $-PARAMETER
        ...
        for I in range(int(v_['1']),int(v_['N'])+1):
            ...
            self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['P*-1-2/H2']))
        #%%%%%%%%%%%%%%%%%% OBJECT BOUNDS %%%%%%%%%%%%%%%%%
#    Solution
# LO SOLTN                ??
    ...
    def eSIN(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, jnp.cos(0.01*EV_[0]))
            if nargout>2:
                H_ = jnp.zeros((1,1))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), -0.01*jnp.sin(0.01*EV_[0]))
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
