...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        for J in range(int(v_['1']),int(v_['P'])+1):
            self.xlower = jtu.np_like_set(self.xlower, jnp.array([ix_['U'+str(int(v_['1']))+','+str(J)]]), 1.0)
            self.xupper = jtu.np_like_set(self.xupper, jnp.array([ix_['U'+str(int(v_['1']))+','+str(J)]]), 1.0)
        ...
        for I in range(int(v_['2']),int(v_['P-1'])+1):
            self.xlower = jtu.np_like_set(self.xlower, jnp.array([ix_['U'+str(I)+','+str(int(v_['P']))]]), 1.0)
            self.xupper = jtu.np_like_set(self.xupper, jnp.array([ix_['U'+str(I)+','+str(int(v_['P']))]]), 1.0)
        ...
        for I in range(int(v_['1']),int(v_['P'])+1):
            for J in range(int(v_['1']),int(v_['P'])+1):
                ...
                v_['UIJ'] = 1.0+v_['MX1X2']
                self.x0 = jtu.np_like_set(self.x0, jnp.array([ix_['U'+str(I)+','+str(J)]]), float(v_['UIJ']))
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
        ...
        for I in range(int(v_['2']),int(v_['P-1'])+1):
            ...
            for J in range(int(v_['2']),int(v_['P-1'])+1):
                ...
                self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['1/H2']))
                posel = len(self.grelt[ig])
                self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['US'+str(I)+','+str(J)])
                nlc = jnp.union1d(nlc,jnp.array([ig]))
