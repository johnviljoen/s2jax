...
# IE N                   20             $-PARAMETER
        ...
        for I in range(int(v_['1']),int(v_['N'])+1):
            ...
            self.gscale = jtu.arrset(self.gscale,ig,float(100000.0))
        ...
        self.gconst = jtu.arrset(self.gconst,ig_['G'+str(int(v_['M']))],float(0.25))
        #%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        for I in range(int(v_['1']),int(v_['N'])+1):
            v_['REALI'] = float(I)
            if('X'+str(I) in ix_):
                self.x0 = jtu.np_like_set(self.x0, ix_['X'+str(I)], float(v_['REALI']))
            else:
                self.y0  = (                       jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X'+str(I)]),float(v_['REALI'])))
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
