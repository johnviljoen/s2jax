...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        elftp = []
        elftp = jtu.loaset(elftp,it,0,'PIJ')
        #%%%%%%%%%%%%%%%%%% ELEMENT USES %%%%%%%%%%%%%%%%%%
        ie_ = {}
        ...
        for I in range(int(v_['1']),int(v_['L'])+1):
            for J in range(int(v_['1']),int(v_['NN'])+1):
                ...
                vname = 'X'+str(J)
                [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,float(0.0),float(1.0),float(0.2))
                posev = jnp.where(elftv[ielftype[ie]]=='V1')[0]
                self.elvar = jtu.loaset(self.elvar,ie,posev[0],iv)
                posep = jnp.where(elftp[ielftype[ie]]=='PIJ')[0]
                self.elpar = jtu.loaset(self.elpar,ie,posep[0],float(v_['P'+str(I)+','+str(J)]))
        #%%%%%%%%%%%%%%%%%%%%% GRFTYPE %%%%%%%%%%%%%%%%%%%%
        igt_ = {}
        [it,igt_,_] = jtu.s2mpj_ii('gNEXP',igt_)
        [it,igt_,_] = jtu.s2mpj_ii('gNEXP',igt_)
        grftp = []
        grftp = jtu.loaset(grftp,it,0,'CI')
        #%%%%%%%%%%%%%%%%%%% GROUP USES %%%%%%%%%%%%%%%%%%%
        self.grelt   = []
        ...
        for I in range(int(v_['1']),int(v_['L'])+1):
            ig = ig_['OBJ'+str(I)]
            self.grftype = jtu.arrset(self.grftype,ig,'gNEXP')
            for J in range(int(v_['1']),int(v_['NN'])+1):
                ig = ig_['OBJ'+str(I)]
                posel = len(self.grelt[ig])
                self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['E'+str(I)+','+str(J)])
                self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['A'+str(I)+','+str(J)]))
            ig = ig_['OBJ'+str(I)]
            posgp = jnp.where(grftp[igt_[self.grftype[ig]]]=='CI')[0]
            self.grpar =jtu.loaset(self.grpar,ig,posgp[0],float(v_['C'+str(I)]))
        #%%%%%%%%%%%%%%%%%% OBJECT BOUNDS %%%%%%%%%%%%%%%%%
#    Solution
...
# **********************
    ...
    def gNEXP(self,nargout,*args):
        ...
        GVAR_ = args[0]
        igr_  = args[1]
        f_= self.grpar[igr_][0]*jnp.exp(-GVAR_)
        if nargout>1:
            g_ = -self.grpar[igr_][0]*jnp.exp(-GVAR_)
            if nargout>2:
                H_ = jnp.zeros((1,1))
                H_ = self.grpar[igr_][0]*jnp.exp(-GVAR_)
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
