...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        for I in range(int(v_['1']),int(v_['N'])+1):
            for K in range(int(v_['1']),int(v_['L'])+1):
                for J in range(int(v_['1']),int(v_['M'])+1):
                    [iv,ix_,_] = jtu.s2mpj_ii('X'+str(I)+','+str(K)+','+str(J),ix_)
                    self.xnames=jtu.arrset(self.xnames,iv,'X'+str(I)+','+str(K)+','+str(J))
        for S in range(int(v_['1']),int(v_['T'])+1):
            for I in range(int(v_['1']),int(v_['N'])+1):
                [iv,ix_,_] = jtu.s2mpj_ii('KINF'+str(I)+','+str(S),ix_)
                self.xnames=jtu.arrset(self.xnames,iv,'KINF'+str(I)+','+str(S))
                [iv,ix_,_] = jtu.s2mpj_ii('PHI'+str(I)+','+str(S),ix_)
                self.xnames=jtu.arrset(self.xnames,iv,'PHI'+str(I)+','+str(S))
            [iv,ix_,_] = jtu.s2mpj_ii('KEFF'+str(S),ix_)
            self.xnames=jtu.arrset(self.xnames,iv,'KEFF'+str(S))
        #%%%%%%%%%%%%%%%%%%  DATA GROUPS %%%%%%%%%%%%%%%%%%%
        ...
        icA  = jtu.append(icA,[ix_['KEFF'+str(int(v_['T']))]])
        valA = jtu.append(valA,float(-1.0))
        for K in range(int(v_['1']),int(v_['L'])+1):
            for J in range(int(v_['1']),int(v_['M'])+1):
                for I in range(int(v_['1']),int(v_['N'])+1):
                    [ig,ig_,_] = jtu.s2mpj_ii('SUMI'+str(K)+','+str(J),ig_)
                    gtype = jtu.arrset(gtype,ig,'==')
                    cnames = jtu.arrset(cnames,ig,'SUMI'+str(K)+','+str(J))
                    irA  = jtu.append(irA,[ig])
                    icA  = jtu.append(icA,[ix_['X'+str(I)+','+str(K)+','+str(J)]])
                    valA = jtu.append(valA,float(v_['V'+str(I)]))
        for I in range(int(v_['1']),int(v_['N'])+1):
            for K in range(int(v_['1']),int(v_['L'])+1):
                for J in range(int(v_['1']),int(v_['M'])+1):
                    [ig,ig_,_] = jtu.s2mpj_ii('SUMLM'+str(I),ig_)
                    gtype = jtu.arrset(gtype,ig,'==')
                    cnames = jtu.arrset(cnames,ig,'SUMLM'+str(I))
                    irA  = jtu.append(irA,[ig])
                    icA  = jtu.append(icA,[ix_['X'+str(I)+','+str(K)+','+str(J)]])
                    valA = jtu.append(valA,float(1.0))
        for I in range(int(v_['1']),int(v_['N'])+1):
            [ig,ig_,_] = jtu.s2mpj_ii('PLAC'+str(I),ig_)
            gtype = jtu.arrset(gtype,ig,'==')
            cnames = jtu.arrset(cnames,ig,'PLAC'+str(I))
            irA  = jtu.append(irA,[ig])
            icA  = jtu.append(icA,[ix_['KINF'+str(I)+','+str(int(v_['1']))]])
            valA = jtu.append(valA,float(-1.0))
            for J in range(int(v_['1']),int(v_['M'])+1):
                [ig,ig_,_] = jtu.s2mpj_ii('PLAC'+str(I),ig_)
                gtype = jtu.arrset(gtype,ig,'==')
                cnames = jtu.arrset(cnames,ig,'PLAC'+str(I))
                irA  = jtu.append(irA,[ig])
                icA  = jtu.append(icA,[ix_['X'+str(I)+','+str(int(v_['1']))+','+str(J)]])
                valA = jtu.append(valA,float(v_['KFRESH']))
        for I in range(int(v_['1']),int(v_['N'])+1):
            for S in range(int(v_['1']),int(v_['T'])+1):
                [ig,ig_,_] = jtu.s2mpj_ii('KERN'+str(I)+','+str(S),ig_)
                gtype = jtu.arrset(gtype,ig,'==')
                cnames = jtu.arrset(cnames,ig,'KERN'+str(I)+','+str(S))
        for I in range(int(v_['1']),int(v_['N'])+1):
            for S in range(int(v_['1']),int(v_['T-1'])+1):
                v_['R'] = 1+S
                [ig,ig_,_] = jtu.s2mpj_ii('KINFF'+str(I)+','+str(S),ig_)
                gtype = jtu.arrset(gtype,ig,'==')
                cnames = jtu.arrset(cnames,ig,'KINFF'+str(I)+','+str(S))
                irA  = jtu.append(irA,[ig])
                icA  = jtu.append(icA,[ix_['KINF'+str(I)+','+str(int(v_['R']))]])
                valA = jtu.append(valA,float(-1.0))
                irA  = jtu.append(irA,[ig])
                icA  = jtu.append(icA,[ix_['KINF'+str(I)+','+str(S)]])
                valA = jtu.append(valA,float(1.0))
        for S in range(int(v_['1']),int(v_['T'])+1):
            [ig,ig_,_] = jtu.s2mpj_ii('CPOW'+str(S),ig_)
            gtype = jtu.arrset(gtype,ig,'==')
            cnames = jtu.arrset(cnames,ig,'CPOW'+str(S))
        for I in range(int(v_['1']),int(v_['N'])+1):
            for S in range(int(v_['1']),int(v_['T'])+1):
                [ig,ig_,_] = jtu.s2mpj_ii('PEAK'+str(I)+','+str(S),ig_)
                gtype = jtu.arrset(gtype,ig,'<=')
                cnames = jtu.arrset(cnames,ig,'PEAK'+str(I)+','+str(S))
        #%%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
        self.n   = len(ix_)
        ngrp   = len(ig_)
        ...
        for K in range(int(v_['1']),int(v_['L'])+1):
            for J in range(int(v_['1']),int(v_['M'])+1):
                self.gconst = jtu.arrset(self.gconst,ig_['SUMI'+str(K)+','+str(J)],float(1.0))
        for I in range(int(v_['1']),int(v_['N'])+1):
            self.gconst = jtu.arrset(self.gconst,ig_['SUMLM'+str(I)],float(1.0))
        for S in range(int(v_['1']),int(v_['T'])+1):
            self.gconst = jtu.arrset(self.gconst,ig_['CPOW'+str(S)],float(1.0))
        v_['TEMP'] = 0.0
        for I in range(int(v_['1']),int(v_['N'])+1):
        ...
        for I in range(int(v_['1']),int(v_['N'])+1):
            for K in range(int(v_['1']),int(v_['L'])+1):
                for J in range(int(v_['1']),int(v_['M'])+1):
                    self.xupper = jtu.np_like_set(self.xupper, jnp.array([ix_['X'+str(I)+','+str(K)+','+str(J)]]), 1.0)
        for I in range(int(v_['1']),int(v_['N'])+1):
            for S in range(int(v_['1']),int(v_['T'])+1):
                self.xupper = jtu.np_like_set(self.xupper, jnp.array([ix_['KINF'+str(I)+','+str(S)]]), v_['KFRESH'])
        v_['LOuKEFF'] = 0.9
        v_['UPuKEFF'] = 1.5
        v_['TEMP'] = float(v_['T'])
        ...
        for S in range(int(v_['1']),int(v_['T'])+1):
            self.xlower = jtu.np_like_set(self.xlower, ix_['KEFF'+str(S)], v_['LOuKEFF'])
            self.xupper = jtu.np_like_set(self.xupper, ix_['KEFF'+str(S)], v_['UPuKEFF'])
        #%%%%%%%%%%%%%%%%%%% START POINT %%%%%%%%%%%%%%%%%%
        ...
        for S in range(int(v_['1']),int(v_['T'])+1):
            for I in range(int(v_['1']),int(v_['N'])+1):
                ...
                posel = len(self.grelt[ig])
                self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['KTP'+str(I)+','+str(S)])
                nlc = jnp.union1d(nlc,jnp.array([ig]))
                self.grelw = jtu.loaset(self.grelw,ig,posel,float(-1.0))
                v_['TEMP'] = v_['NROW'+str(I)]
                v_['NuROW'] = int(jnp.fix(v_['TEMP']))
        ...
        for I in range(int(v_['1']),int(v_['N'])+1):
            for S in range(int(v_['1']),int(v_['T-1'])+1):
                ...
                posel = len(self.grelt[ig])
                self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['P'+str(I)+','+str(S)])
                nlc = jnp.union1d(nlc,jnp.array([ig]))
                self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['-ACC']))
        for S in range(int(v_['1']),int(v_['T'])+1):
            for I in range(int(v_['1']),int(v_['N'])+1):
                ig = ig_['CPOW'+str(S)]
                posel = len(self.grelt[ig])
                self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['P'+str(I)+','+str(S)])
                nlc = jnp.union1d(nlc,jnp.array([ig]))
                self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['V'+str(I)]))
        for I in range(int(v_['1']),int(v_['N'])+1):
            for S in range(int(v_['1']),int(v_['T'])+1):
