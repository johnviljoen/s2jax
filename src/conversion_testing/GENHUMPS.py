...
# RE ZETA                2.0            $-PARAMETER
        ...
        #%%%%%%%%%%%%%%%%%% START POINT %%%%%%%%%%%%%%%%%%
        self.x0 = jnp.full((self.n,1),float(-506.2))
        self.x0 = jtu.np_like_set(self.x0, ix_['X'+str(int(v_['1']))], float(-506.0))
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
        ...
        for I in range(int(v_['1']),int(v_['N-1'])+1):
            ...
            vname = 'X'+str(I)
            [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,None,None,float(-506.2))
        ...
        for I in range(int(v_['1']),int(v_['N-1'])+1):
            ...
            posel = len(self.grelt[ig])
            self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['SX'+str(I)])
            self.grelw = jtu.loaset(self.grelw,ig,posel,float(0.05))
            posel = posel+1
            self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['SY'+str(I)])
            self.grelw = jtu.loaset(self.grelw,ig,posel,float(0.05))
        #%%%%%%%%%%%%%%%%%% OBJECT BOUNDS %%%%%%%%%%%%%%%%%
        self.objlower = 0.0
# LO SOLTN               0.0
