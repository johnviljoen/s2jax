...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        if('B1' in ix_):
            ...
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['B1']),float(0.1)))
        if('B2' in ix_):
            self.x0 = jtu.np_like_set(self.x0, ix_['B2'], float(0.01))
        else:
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['B2']),float(0.01)))
        if('B3' in ix_):
            self.x0 = jtu.np_like_set(self.x0, ix_['B3'], float(0.02))
        else:
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['B3']),float(0.02)))
        pass
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
        [it,iet_,_] = jtu.s2mpj_ii( 'eE16', iet_)
        ...
        for I in range(int(v_['1']),int(v_['M'])+1):
            ...
            self.elftype = jtu.arrset(self.elftype,ie,'eE16')
            ielftype = jtu.arrset(ielftype,ie,iet_["eE16"])
            vname = 'B1'
...
# **********************
    ...
    def eE16(self, nargout,*args):
        ...
        EV_  = args[0]
        iel_ = args[1]
        E = jnp.exp(-EV_[0]*self.elpar[iel_][0])
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, -EX/V2PV3X)
            g_ = jtu.np_like_set(g_, 1, -E/V2PV32)
            g_ = jtu.np_like_set(g_, 2, -EX/V2PV32)
            if nargout>2:
                H_ = jnp.zeros((3,3))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), EX2/V2PV3X)
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), EX/V2PV32)
                H_ = jtu.np_like_set(H_, jnp.array([1,0]), H_[0,1])
                H_ = jtu.np_like_set(H_, jnp.array([0,2]), EX2/V2PV32)
                H_ = jtu.np_like_set(H_, jnp.array([2,0]), H_[0,2])
                H_ = jtu.np_like_set(H_, jnp.array([1,1]), 2.0*E/V2PV33)
                H_ = jtu.np_like_set(H_, jnp.array([1,2]), 2.0*EX/V2PV33)
                H_ = jtu.np_like_set(H_, jnp.array([2,1]), H_[1,2])
                H_ = jtu.np_like_set(H_, jnp.array([2,2]), 2.0*EX2/V2PV33)
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
