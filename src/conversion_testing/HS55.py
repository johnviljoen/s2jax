...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        self.gconst = jtu.arrset(self.gconst,ig_['CON2'],float(3.0))
        self.gconst = jtu.arrset(self.gconst,ig_['CON3'],float(2.0))
        ...
        self.xupper = jtu.np_like_set(self.xupper, ix_['X4'], 1.0)
        #%%%%%%%%%%%%%%%%%% START POINT %%%%%%%%%%%%%%%%%%
        ...
        iet_  = {}
        elftv = []
        [it,iet_,_] = jtu.s2mpj_ii( 'eEPROD', iet_)
        ...
        self.elftype = jtu.arrset(self.elftype,ie,'eEPROD')
        ielftype = jtu.arrset(ielftype,ie,iet_["eEPROD"])
        vname = 'X1'
    ...
    def eEPROD(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, EV_[1]*PROD)
            g_ = jtu.np_like_set(g_, 1, EV_[0]*PROD)
            if nargout>2:
                H_ = jnp.zeros((2,2))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), EV_[1]*EV_[1]*PROD)
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), PROD+EV_[0]*EV_[1]*PROD)
                H_ = jtu.np_like_set(H_, jnp.array([1,0]), H_[0,1])
                H_ = jtu.np_like_set(H_, jnp.array([1,1]), EV_[0]*EV_[0]*PROD)
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
