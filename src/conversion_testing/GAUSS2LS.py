...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        self.x0 = jtu.np_like_set(self.x0, ix_['B1'], float(96.0))
        self.x0 = jtu.np_like_set(self.x0, ix_['B2'], float(0.009))
        self.x0 = jtu.np_like_set(self.x0, ix_['B3'], float(103.0))
        self.x0 = jtu.np_like_set(self.x0, ix_['B4'], float(106.0))
        self.x0 = jtu.np_like_set(self.x0, ix_['B5'], float(18.0))
        self.x0 = jtu.np_like_set(self.x0, ix_['B6'], float(72.0))
        self.x0 = jtu.np_like_set(self.x0, ix_['B7'], float(151.0))
        self.x0 = jtu.np_like_set(self.x0, ix_['B8'], float(18.0))
        pass
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
        ...
        [it,iet_,_] = jtu.s2mpj_ii( 'eE17', iet_)
        elftv = jtu.loaset(elftv,it,0,'V1')
        elftv = jtu.loaset(elftv,it,1,'V2')
        ...
        for I in range(int(v_['1']),int(v_['M'])+1):
            ...
            ename = 'EB'+str(I)
            [ie,ie_,_] = jtu.s2mpj_ii(ename,ie_)
            self.elftype = jtu.arrset(self.elftype,ie,'eE17')
            ielftype = jtu.arrset(ielftype,ie,iet_["eE17"])
            vname = 'B3'
            [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,None,None,None)
            posev = jnp.where(elftv[ielftype[ie]]=='V1')[0]
            self.elvar = jtu.loaset(self.elvar,ie,posev[0],iv)
...
# **********************
    ...
    def eE17(self, nargout,*args):
        ...
        if nargout>1:
            ...
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, E)
            g_ = jtu.np_like_set(g_, 1, -V1E*TV2MXV)
            g_ = jtu.np_like_set(g_, 2, TV1E*V2MX2/V33)
            if nargout>2:
                H_ = jnp.zeros((3,3))
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), -E*TV2MXV)
                H_ = jtu.np_like_set(H_, jnp.array([1,0]), H_[0,1])
                H_ = jtu.np_like_set(H_, jnp.array([0,2]), E*TV2MX2/V33)
                H_ = jtu.np_like_set(H_, jnp.array([2,0]), H_[0,2])
                H_ = jtu.np_like_set(H_, jnp.array([1,1]), TV1E*(TV2MXW-1.0)/V32)
                H_ = jtu.np_like_set(H_, jnp.array([1,2]), TV1E*TV2MX*(1.0-V2MX2/V32)/V33)
                H_ = jtu.np_like_set(H_, jnp.array([2,1]), H_[1,2])
                H_ = jtu.np_like_set(H_, jnp.array([2,2]), TV1E*V2MX2*(TV2MXW-3.0)/EV_[2]**4)
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
