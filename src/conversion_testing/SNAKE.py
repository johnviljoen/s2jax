...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        irA  = jtu.append(irA,[ig])
        icA  = jtu.append(icA,[ix_['X']])
        valA = jtu.append(valA,float(v_['TIP']))
        #%%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
        self.n   = len(ix_)
        ngrp   = len(ig_)
        ...
        if('X' in ix_):
            self.x0 = jtu.np_like_set(self.x0, ix_['X'], float(1.0))
        else:
            self.y0  = (                   jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X']),float(1.0)))
        if('Y' in ix_):
            self.x0 = jtu.np_like_set(self.x0, ix_['Y'], float(5.0))
        else:
            self.y0  = (                   jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['Y']),float(5.0)))
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
        ...
        self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['SINX'])
...
# **********************
    ...
    def eSINE(self, nargout,*args):
        ...
        if nargout>1:
            ...
            if nargout>2:
                H_ = jnp.zeros((1,1))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), -SV)
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
