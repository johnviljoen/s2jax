...
# IE N                   10000          $-PARAMETER
        ...
        for I in range(int(v_['1']),int(v_['N-2'])+1):
            ...
            self.elftype = jtu.arrset(self.elftype,ie,'eTG')
            ielftype = jtu.arrset(ielftype,ie,iet_["eTG"])
            vname = 'X'+str(I)
            ...
            self.elvar = jtu.loaset(self.elvar,ie,posev[0],iv)
            posep = jnp.where(elftp[ielftype[ie]]=='AP')[0]
            self.elpar = jtu.loaset(self.elpar,ie,posep[0],float(v_['10/N-2']))
        #%%%%%%%%%%%%%%%%%%% GROUP USES %%%%%%%%%%%%%%%%%%%
        self.grelt   = []
    ...
    def eTG(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, -AU2SQ*EXPA1)
            g_ = jtu.np_like_set(g_, 1, -AU2SQ*EXPA2+2.0*IV_[1]*TMEXPA)
            g_ =  U_.T.dot(g_)
            if nargout>2:
                H_ = jnp.zeros((2,2))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), -AU2SQ*EXPA11)
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), -AU2SQ*EXPA12-2.0*IV_[1]*EXPA1)
                H_ = jtu.np_like_set(H_, jnp.array([1,0]), H_[0,1])
                H_ = jtu.np_like_set(H_, jnp.array([1,1]), -AU2SQ*EXPA22-4.0*IV_[1]*EXPA2+2.0*TMEXPA)
