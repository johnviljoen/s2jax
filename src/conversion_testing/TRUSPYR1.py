...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        for I in range(int(v_['1']),int(v_['NDIM'])+1):
            [ig,ig_,_] = jtu.s2mpj_ii('STREN',ig_)
            gtype = jtu.arrset(gtype,ig,'<=')
            cnames = jtu.arrset(cnames,ig,'STREN')
            irA  = jtu.append(irA,[ig])
            icA  = jtu.append(icA,[ix_['DISPL'+str(I)]])
            valA = jtu.append(valA,float(v_['Q'+str(I)]))
        #%%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
        self.n   = len(ix_)
        ngrp   = len(ig_)
        ...
        self.gconst = jtu.arrset(self.gconst,ig_['STREN'],float(v_['ALPHA']))
        #%%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
