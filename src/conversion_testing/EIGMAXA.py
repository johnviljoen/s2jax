...
# 
#    The problem is, given a symmetric matrix A, to jtu.find a unit vector
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        gtype        = jnp.array([])
        [ig,ig_,_] = jtu.s2mpj_ii('MAXEIG',ig_)
        ...
        for I in range(int(v_['1']),int(v_['N'])+1):
            for K in range(int(v_['1']),int(v_['N'])+1):
                ...
                irA  = jtu.append(irA,[ig])
                icA  = jtu.append(icA,[ix_['Q'+str(K)]])
                valA = jtu.append(valA,float(v_['-AIK']))
        #%%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
        self.n   = len(ix_)
        ngrp   = len(ig_)
        ...
        self.gconst = jtu.arrset(self.gconst,ig_['O'],float(1.0))
        #%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        for I in range(int(v_['1']),int(v_['N'])+1):
            self.x0 = jtu.np_like_set(self.x0, ix_['Q'+str(I)], float(v_['1/ROOTN']))
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
        ...
        for K in range(int(v_['1']),int(v_['N'])+1):
            ig = ig_['O']
            posel = len(self.grelt[ig])
            self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['O'+str(K)])
            nlc = jnp.union1d(nlc,jnp.array([ig]))
            self.grelw = jtu.loaset(self.grelw,ig,posel,1.)
