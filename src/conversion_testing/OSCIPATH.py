...
# RE RHO                 1.0            $-PARAMETER    Nesterov's original value
        ...
        self.gconst = jtu.arrset(self.gconst,ig_['Q1'],float(1.0))
        #%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        iet_  = {}
        elftv = []
        [it,iet_,_] = jtu.s2mpj_ii( 'eCHEB', iet_)
        elftv = jtu.loaset(elftv,it,0,'TAU')
        #%%%%%%%%%%%%%%%%%% ELEMENT USES %%%%%%%%%%%%%%%%%%
        ie_ = {}
        ...
        for I in range(int(v_['2']),int(v_['N'])+1):
            ...
            if newelt:
                self.elftype = jtu.arrset(self.elftype,ie,'eCHEB')
                ielftype = jtu.arrset(ielftype,ie,iet_['eCHEB'])
            vname = 'X'+str(int(v_['I-1']))
            [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,None,None,None)
            posev = jnp.where(elftv[ielftype[ie]]=='TAU')[0]
            self.elvar = jtu.loaset(self.elvar,ie,posev[0],iv)
        #%%%%%%%%%%%%%%%%%%%%% GRFTYPE %%%%%%%%%%%%%%%%%%%%
        igt_ = {}
        [it,igt_,_] = jtu.s2mpj_ii('gPL2',igt_)
        [it,igt_,_] = jtu.s2mpj_ii('gPL2',igt_)
        grftp = []
        ...
        for ig in range(0,ngrp):
            self.grftype = jtu.arrset(self.grftype,ig,'gPL2')
        ig = ig_['Q1']
        posgp = jnp.where(grftp[igt_[self.grftype[ig]]]=='P')[0]
        self.grpar =jtu.loaset(self.grpar,ig,posgp[0],float(0.25))
        for I in range(int(v_['2']),int(v_['N'])+1):
            ig = ig_['Q'+str(I)]
            ...
            posgp = jnp.where(grftp[igt_[self.grftype[ig]]]=='P')[0]
            self.grpar =jtu.loaset(self.grpar,ig,posgp[0],float(v_['RHO']))
        #%%%%%%%%%%%%%%%%%% OBJECT BOUNDS %%%%%%%%%%%%%%%%%
        self.objlower = 0.0
#    Solution
# LO SOLTN                0.0
    ...
    def eCHEB(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, 4.0*EV_[0])
            if nargout>2:
                H_ = jnp.zeros((1,1))
