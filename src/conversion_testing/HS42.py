...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        [it,iet_,_] = jtu.s2mpj_ii( 'eSQMP', iet_)
        elftv = jtu.loaset(elftv,it,0,'V1')
        elftp = []
        ...
        for I in range(int(v_['1']),int(v_['N'])+1):
            ...
            self.elftype = jtu.arrset(self.elftype,ie,'eSQMP')
            ielftype = jtu.arrset(ielftype,ie,iet_["eSQMP"])
            vname = 'X'+str(I)
            [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,None,None,float(1.0))
            posev = jnp.where(elftv[ielftype[ie]]=='V1')[0]
            self.elvar = jtu.loaset(self.elvar,ie,posev[0],iv)
            posep = jnp.where(elftp[ielftype[ie]]=='P')[0]
            self.elpar = jtu.loaset(self.elpar,ie,posep[0],float(v_['PAR']))
        ename = 'E5'
        [ie,ie_,_] = jtu.s2mpj_ii(ename,ie_)
...
# **********************
    ...
    @staticmethod
    def eSQMP(self, nargout,*args):

        import jax.numpy as jnp
        EV_  = args[0]
        iel_ = args[1]
        VMP = EV_[0]-self.elpar[iel_][0]
        f_   = VMP*VMP
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, VMP+VMP)
            if nargout>2:
                H_ = jnp.zeros((1,1))
