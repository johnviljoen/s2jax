...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        for K in range(int(v_['-LGSG']),int(v_['LGTR'])+1):
            [iv,ix_,_] = jtu.s2mpj_ii('C'+str(K),ix_)
            self.xnames=jtu.arrset(self.xnames,iv,'C'+str(K))
        for I in range(int(v_['1']),int(v_['LGSG'])+1):
            [iv,ix_,_] = jtu.s2mpj_ii('SG'+str(I),ix_)
            self.xnames=jtu.arrset(self.xnames,iv,'SG'+str(I))
        #%%%%%%%%%%%%%%%%%%  DATA GROUPS %%%%%%%%%%%%%%%%%%%
        ...
        for K in range(int(v_['1']),int(v_['LGTR'])+1):
            [ig,ig_,_] = jtu.s2mpj_ii('R'+str(K),ig_)
            gtype = jtu.arrset(gtype,ig,'<>')
        [ig,ig_,_] = jtu.s2mpj_ii('ENERG',ig_)
        gtype = jtu.arrset(gtype,ig,'==')
        cnames = jtu.arrset(cnames,ig,'ENERG')
        #%%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
        self.n   = len(ix_)
        ngrp   = len(ig_)
        ...
        for K in range(int(v_['1']),int(v_['LGTR'])+1):
            self.gconst = jtu.arrset(self.gconst,ig_['R'+str(K)],float(v_['TR'+str(K)]))
        self.gconst = jtu.arrset(self.gconst,ig_['ENERG'],float(v_['PIC']))
        #%%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        self.xlower = jnp.zeros((self.n,1))
        self.xupper = jnp.full((self.n,1),float('inf'))
        for K in range(int(v_['-LGSG']),int(v_['0'])+1):
            self.xlower = jtu.np_like_set(self.xlower, ix_['C'+str(K)], 0.0)
            self.xupper = jtu.np_like_set(self.xupper, ix_['C'+str(K)], 0.0)
        #%%%%%%%%%%%%%%%%%%% START POINT %%%%%%%%%%%%%%%%%%
        self.x0 = jnp.zeros((self.n,1))
        self.y0 = jnp.zeros((self.m,1))
        for K in range(int(v_['1']),int(v_['LGTR'])+1):
            self.x0 = jtu.np_like_set(self.x0, ix_['C'+str(K)], float(v_['CC'+str(K)]))
        for I in range(int(v_['1']),int(v_['LGSG'])+1):
            self.x0 = jtu.np_like_set(self.x0, ix_['SG'+str(I)], float(v_['SSG'+str(I)]))
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
        ...
        elftp = []
        elftp = jtu.loaset(elftp,it,0,'IDX')
        ...
        for K in range(int(v_['1']),int(v_['LGTR'])+1):
            for I in range(int(v_['1']),int(v_['LGSG'])+1):
                ...
                self.elvar = jtu.loaset(self.elvar,ie,posev[0],iv)
                posep = jnp.where(elftp[ielftype[ie]]=='IDX')[0]
                self.elpar = jtu.loaset(self.elpar,ie,posep[0],float(v_['RIDX']))
        for I in range(int(v_['1']),int(v_['LGSG'])+1):
            ename = 'ESQ'+str(I)
            [ie,ie_,_] = jtu.s2mpj_ii(ename,ie_)
        ...
        for K in range(int(v_['1']),int(v_['LGTR'])+1):
            ...
            for I in range(int(v_['1']),int(v_['LGSG'])+1):
                ...
                self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['PROD'+str(K)+','+str(I)])
                nlc = jnp.union1d(nlc,jnp.array([ig]))
                self.grelw = jtu.loaset(self.grelw,ig,posel,1.)
        for I in range(int(v_['1']),int(v_['LGSG'])+1):
            ig = ig_['ENERG']
            posel = len(self.grelt[ig])
            self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['ESQ'+str(I)])
            nlc = jnp.union1d(nlc,jnp.array([ig]))
            self.grelw = jtu.loaset(self.grelw,ig,posel,1.)
    ...
    def ePR(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, SCAL*EV_[1])
            g_ = jtu.np_like_set(g_, 1, SCAL*EV_[0])
            if nargout>2:
                H_ = jnp.zeros((2,2))
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), SCAL)
