...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        for I in range(int(v_['1']),int(v_['M'])+1):
            v_['LOGY'] = jnp.log(v_['Y'+str(I)])
            self.gconst = jtu.arrset(self.gconst,ig_['F'+str(I)],float(v_['LOGY']))
        #%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        if('B1' in ix_):
            ...
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['B1']),float(2.0)))
        if('B2' in ix_):
            self.x0 = jtu.np_like_set(self.x0, ix_['B2'], float(0.0001))
        else:
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['B2']),float(0.0001)))
        if('B3' in ix_):
            self.x0 = jtu.np_like_set(self.x0, ix_['B3'], float(-0.01))
        else:
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['B3']),float(-0.01)))
        pass
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
        ...
        elftp = []
        elftp = jtu.loaset(elftp,it,0,'X1')
        elftp = jtu.loaset(elftp,it,1,'X2')
        #%%%%%%%%%%%%%%%%%% ELEMENT USES %%%%%%%%%%%%%%%%%%
        ie_ = {}
        ...
        for I in range(int(v_['1']),int(v_['M'])+1):
            ...
            self.elvar = jtu.loaset(self.elvar,ie,posev[0],iv)
            posep = jnp.where(elftp[ielftype[ie]]=='X1')[0]
            self.elpar = jtu.loaset(self.elpar,ie,posep[0],float(v_['X1'+str(I)]))
            posep = jnp.where(elftp[ielftype[ie]]=='X2')[0]
            self.elpar = jtu.loaset(self.elpar,ie,posep[0],float(v_['X2'+str(I)]))
        #%%%%%%%%%%%%%%%%%%% GROUP USES %%%%%%%%%%%%%%%%%%%
        self.grelt   = []
...
# **********************
    ...
    def eE6(self, nargout,*args):
        ...
        EV_  = args[0]
        iel_ = args[1]
        E = jnp.exp(-EV_[1]*self.elpar[iel_][1])
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, X1E)
            g_ = jtu.np_like_set(g_, 1, -V1X1E*self.elpar[iel_][1])
            if nargout>2:
                H_ = jnp.zeros((2,2))
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), -X1E*self.elpar[iel_][1])
                H_ = jtu.np_like_set(H_, jnp.array([1,0]), H_[0,1])
                H_ = jtu.np_like_set(H_, jnp.array([1,1]), V1X1E*self.elpar[iel_][1]**2)
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
