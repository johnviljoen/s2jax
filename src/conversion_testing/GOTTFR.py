    ...
    def eET1(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, -0.1136*(FB-FA))
            g_ = jtu.np_like_set(g_, 1, -0.3408*FB)
            if nargout>2:
                H_ = jnp.zeros((2,2))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), 0.2272)
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), 0.3408)
    ...
    def eET2(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, 15.0*FB)
            g_ = jtu.np_like_set(g_, 1, -7.5*(FB+FA))
            if nargout>2:
                H_ = jnp.zeros((2,2))
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), -15.0)
                H_ = jtu.np_like_set(H_, jnp.array([1,0]), H_[0,1])
                H_ = jtu.np_like_set(H_, jnp.array([1,1]), 15.0)
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
