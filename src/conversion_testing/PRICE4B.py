...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        self.m       = 0
        #%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        self.xlower = jnp.full((self.n,1),-50.0)
        self.xupper = jnp.full((self.n,1),50.0)
        #%%%%%%%%%%%%%%%%%%% START POINT %%%%%%%%%%%%%%%%%%
        ...
        elftv = jtu.loaset(elftv,it,0,'X')
        [it,iet_,_] = jtu.s2mpj_ii( 'eCUBEL', iet_)
        ...
        self.elftype = jtu.arrset(self.elftype,ie,'eCUBEL')
        ielftype = jtu.arrset(ielftype,ie,iet_["eCUBEL"])
        vname = 'X1'
        [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,float(-50.0),float(50.0),None)
    ...
    def eCUBEL(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, 3.0*EV_[1]*EV_[0]**2)
            g_ = jtu.np_like_set(g_, 1, EV_[0]**3)
            if nargout>2:
                H_ = jnp.zeros((2,2))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), 6.0*EV_[1]*EV_[0])
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), 3.0*EV_[0]**2)
