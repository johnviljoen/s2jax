    ...
    def eH(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, EV_[1]*R2*R3-R1*R3+R1*R2*DR3DX)
            g_ = jtu.np_like_set(g_, 1, EV_[0]*R2*R3-R1*R2)
            if nargout>2:
                H_ = jnp.zeros((2,2))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), -2.0*EV_[1]*R3-2.0*R1*DR3DX+EV_[1]*R2*DR3DX+R1*R2*DR3DX2)
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), R2*R3+EV_[0]*R2*DR3DX-EV_[1]*R2+R1-EV_[0]*R3)
                H_ = jtu.np_like_set(H_, jnp.array([1,0]), H_[0,1])
                H_ = jtu.np_like_set(H_, jnp.array([1,1]), -2.0*EV_[0]*R2)
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
