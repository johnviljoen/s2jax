    ...
    def __init__(self, *args): 
        ...
        for I in range(int(v_['1']),int(v_['LB'])+1):
            ...
            for J in range(int(v_['1']),int(v_['I-1'])+1):
                ...
                valA = jtu.append(valA,float(v_['KAP']))
        ...
        for I in range(int(v_['1']),int(v_['N'])+1):
            ...
            self.elpar = jtu.loaset(self.elpar,ie,posep[0],float(v_['SCALE'+str(I)]))
            ename = 'Q'+str(I)
            [ie,ie_,newelt] = jtu.s2mpj_ii(ename,ie_)
        ...
        for I in range(int(v_['1']),int(v_['LB'])+1):
            ...
            for J in range(int(v_['1']),int(v_['I-1'])+1):
                ...
                self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['-KAPPA3']))
            ig = ig_['G'+str(I)]
            posel = len(self.grelt[ig])
            self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['Q'+str(I)])
            nlc = jnp.union1d(nlc,jnp.array([ig]))
            self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['KAPPA2']))
            for J in range(int(v_['I+1']),int(v_['I+UB'])+1):
                ig = ig_['G'+str(I)]
                posel = len(self.grelt[ig])
                self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['E'+str(J)])
    ...
    def eCB(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, 3.0*PP*EV_[0]*EV_[0])
            if nargout>2:
                H_ = jnp.zeros((1,1))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), 6.0*PP*EV_[0])
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
