...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        gtype        = jnp.array([])
        for I in range(int(v_['1']),int(v_['16'])+1):
            [ig,ig_,_] = jtu.s2mpj_ii('OG'+str(I),ig_)
        ...
        for I in range(int(v_['1']),int(v_['8'])+1):
            for J in range(int(v_['1']),int(v_['16'])+1):
                ...
                valA = jtu.append(valA,float(v_['B'+str(I)+','+str(J)]))
        #%%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
        self.n   = len(ix_)
        ngrp   = len(ig_)
        ...
        for I in range(int(v_['1']),int(v_['8'])+1):
            self.gconst = jtu.arrset(self.gconst,ig_['G'+str(I)],float(v_['C'+str(I)]))
        #%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        elftp = []
        elftp = jtu.loaset(elftp,it,0,'AIJ')
        #%%%%%%%%%%%%%%%%%% ELEMENT USES %%%%%%%%%%%%%%%%%%
        ie_ = {}
        ...
        for I in range(int(v_['1']),int(v_['16'])+1):
            for J in range(int(v_['1']),int(v_['16'])+1):
                ...
                vname = 'X'+str(I)
                [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,None,float(5.0),float(10.0))
                ...
                self.elvar = jtu.loaset(self.elvar,ie,posev[0],iv)
                posep = jnp.where(elftp[ielftype[ie]]=='AIJ')[0]
                self.elpar = jtu.loaset(self.elpar,ie,posep[0],float(v_['A'+str(I)+','+str(J)]))
        #%%%%%%%%%%%%%%%%%%% GROUP USES %%%%%%%%%%%%%%%%%%%
        self.grelt   = []
    ...
    def ePROD(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, self.elpar[iel_][0]*TU1P1*SECOND)
            g_ = jtu.np_like_set(g_, 1, self.elpar[iel_][0]*TU2P1*FIRST)
            if nargout>2:
                H_ = jnp.zeros((2,2))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), self.elpar[iel_][0]*2.0*SECOND)
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), self.elpar[iel_][0]*TU1P1*TU2P1)
                H_ = jtu.np_like_set(H_, jnp.array([1,0]), H_[0,1])
                H_ = jtu.np_like_set(H_, jnp.array([1,1]), self.elpar[iel_][0]*2.0*FIRST)
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
