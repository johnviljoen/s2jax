    ...
    def __init__(self, *args): 
        ...
        for I in range(int(v_['2']),int(v_['NNODES'])+1):
            ...
            for J in range(int(v_['1']),int(v_['I-1'])+1):
                ...
                v_['AIJ'] = int(jnp.fix(v_['RAIJ']))
                for K in range(int(v_['1']),int(v_['AIJ'])+1):
                    [ig,ig_,_] = jtu.s2mpj_ii('S'+str(J)+','+str(I),ig_)
                    gtype = jtu.arrset(gtype,ig,'==')
                    cnames = jtu.arrset(cnames,ig,'S'+str(J)+','+str(I))
                v_['AIJ-1'] = -1+v_['AIJ']
                v_['NAIJ'] = -1*v_['AIJ-1']
        ...
        for I in range(int(v_['1']),int(v_['NNODES'])+1):
            self.xlower = jtu.np_like_set(self.xlower, ix_['R'+str(I)], 1.0)
        ...
        self.xlower = jtu.np_like_set(self.xlower, ix_['Y'+str(int(v_['1']))], 0.0)
        self.xupper = jtu.np_like_set(self.xupper, ix_['Y'+str(int(v_['1']))], 0.0)
        self.xlower = jtu.np_like_set(self.xlower, ix_['Y'+str(int(v_['2']))], 0.0)
        self.xupper = jtu.np_like_set(self.xupper, ix_['Y'+str(int(v_['2']))], 0.0)
        #%%%%%%%%%%%%%%%%%%% START POINT %%%%%%%%%%%%%%%%%%
        ...
        for I in range(int(v_['1']),int(v_['NNODES'])+1):
            ...
            self.x0 = jtu.np_like_set(self.x0, ix_['R'+str(I)], float(v_['RI']))
            self.x0 = jtu.np_like_set(self.x0, ix_['X'+str(I)], float(v_['RM']))
            self.x0 = jtu.np_like_set(self.x0, ix_['Y'+str(I)], float(v_['RP']))
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
        [it,iet_,_] = jtu.s2mpj_ii( 'eIPSQ', iet_)
        elftv = jtu.loaset(elftv,it,0,'X')
        elftv = jtu.loaset(elftv,it,1,'Y')
        [it,iet_,_] = jtu.s2mpj_ii( 'eIMSQ', iet_)
        elftv = jtu.loaset(elftv,it,0,'X')
        elftv = jtu.loaset(elftv,it,1,'Y')
        #%%%%%%%%%%%%%%%%%% ELEMENT USES %%%%%%%%%%%%%%%%%%
        ie_ = {}
        ...
        for I in range(int(v_['2']),int(v_['NNODES'])+1):
            ...
            for J in range(int(v_['1']),int(v_['I-1'])+1):
                ...
                self.elftype = jtu.arrset(self.elftype,ie,'eIPSQ')
                ielftype = jtu.arrset(ielftype,ie,iet_["eIPSQ"])
                vname = 'R'+str(I)
                ...
                [ie,ie_,_] = jtu.s2mpj_ii(ename,ie_)
                self.elftype = jtu.arrset(self.elftype,ie,'eIMSQ')
                ielftype = jtu.arrset(ielftype,ie,iet_["eIMSQ"])
                vname = 'X'+str(I)
                [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,None,None,None)
                posev = jnp.where(elftv[ielftype[ie]]=='X')[0]
                self.elvar = jtu.loaset(self.elvar,ie,posev[0],iv)
        ...
        for I in range(int(v_['2']),int(v_['NNODES'])+1):
            ...
            for J in range(int(v_['1']),int(v_['I-1'])+1):
                ...
                self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['DR'+str(J)+','+str(I)])
                nlc = jnp.union1d(nlc,jnp.array([ig]))
                self.grelw = jtu.loaset(self.grelw,ig,posel,1.)
                posel = len(self.grelt[ig])
                self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['DX'+str(J)+','+str(I)])
                nlc = jnp.union1d(nlc,jnp.array([ig]))
                self.grelw = jtu.loaset(self.grelw,ig,posel,float(-1.0))
                posel = posel+1
                self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['DY'+str(J)+','+str(I)])
                self.grelw = jtu.loaset(self.grelw,ig,posel,float(-1.0))
        #%%%%%%%%%%%%%%%%%% OBJECT BOUNDS %%%%%%%%%%%%%%%%%
# ZL DISCS                              RNODES
#    Solution
