...
# IE NH                  30000          $-PARAMETER
        ...
        for I in range(int(v_['0']),int(v_['NH'])+1):
            ...
            [iv,ix_,_] = jtu.s2mpj_ii('X1'+str(I),ix_)
            self.xnames=jtu.arrset(self.xnames,iv,'X1'+str(I))
            [iv,ix_,_] = jtu.s2mpj_ii('X2'+str(I),ix_)
            self.xnames=jtu.arrset(self.xnames,iv,'X2'+str(I))
        #%%%%%%%%%%%%%%%%%%  DATA GROUPS %%%%%%%%%%%%%%%%%%%
        ...
        icA  = jtu.append(icA,[ix_['X1'+str(int(v_['NH']))]])
        valA = jtu.append(valA,float(1.0))
        irA  = jtu.append(irA,[ig])
        icA  = jtu.append(icA,[ix_['X2'+str(int(v_['NH']))]])
        valA = jtu.append(valA,float(1.0))
        for I in range(int(v_['0']),int(v_['NH-1'])+1):
            v_['I+1'] = 1+I
            [ig,ig_,_] = jtu.s2mpj_ii('ODE1'+str(I),ig_)
            gtype = jtu.arrset(gtype,ig,'==')
            cnames = jtu.arrset(cnames,ig,'ODE1'+str(I))
            irA  = jtu.append(irA,[ig])
            icA  = jtu.append(icA,[ix_['X1'+str(I)]])
            valA = jtu.append(valA,float(1.0))
            irA  = jtu.append(irA,[ig])
            icA  = jtu.append(icA,[ix_['X1'+str(int(v_['I+1']))]])
            valA = jtu.append(valA,float(-1.0))
            [ig,ig_,_] = jtu.s2mpj_ii('ODE2'+str(I),ig_)
            gtype = jtu.arrset(gtype,ig,'==')
            cnames = jtu.arrset(cnames,ig,'ODE2'+str(I))
            irA  = jtu.append(irA,[ig])
            icA  = jtu.append(icA,[ix_['X2'+str(I)]])
            valA = jtu.append(valA,float(1.0))
            irA  = jtu.append(irA,[ig])
            icA  = jtu.append(icA,[ix_['X2'+str(int(v_['I+1']))]])
            valA = jtu.append(valA,float(-1.0))
        #%%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
        self.n   = len(ix_)
        ngrp   = len(ig_)
        ...
        self.xlower = jtu.np_like_set(self.xlower, ix_['X1'+str(int(v_['0']))], v_['X1u0'])
        self.xupper = jtu.np_like_set(self.xupper, ix_['X1'+str(int(v_['0']))], v_['X1u0'])
        self.xlower = jtu.np_like_set(self.xlower, ix_['X2'+str(int(v_['0']))], v_['X2u0'])
        self.xupper = jtu.np_like_set(self.xupper, ix_['X2'+str(int(v_['0']))], v_['X2u0'])
        #%%%%%%%%%%%%%%%%%%% START POINT %%%%%%%%%%%%%%%%%%
        self.x0 = jnp.zeros((self.n,1))
        self.y0 = jnp.zeros((self.m,1))
        for I in range(int(v_['0']),int(v_['NH'])+1):
            if('U'+str(I) in ix_):
                self.x0 = jtu.np_like_set(self.x0, ix_['U'+str(I)], float(0.0))
            else:
                self.y0  = (                       jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['U'+str(I)]),float(0.0)))
            if('X1'+str(I) in ix_):
                self.x0 = jtu.np_like_set(self.x0, ix_['X1'+str(I)], float(1.0))
            else:
                self.y0  = (                       jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X1'+str(I)]),float(1.0)))
            if('X2'+str(I) in ix_):
                self.x0 = jtu.np_like_set(self.x0, ix_['X2'+str(I)], float(0.0))
            else:
                self.y0  = (                       jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X2'+str(I)]),float(0.0)))
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
        [it,iet_,_] = jtu.s2mpj_ii( 'eDIFSQ', iet_)
        elftv = jtu.loaset(elftv,it,0,'U1')
        elftv = jtu.loaset(elftv,it,1,'U2')
        [it,iet_,_] = jtu.s2mpj_ii( 'eP1', iet_)
        ...
        for I in range(int(v_['0']),int(v_['NH-1'])+1):
            ...
            self.elftype = jtu.arrset(self.elftype,ie,'eDIFSQ')
            ielftype = jtu.arrset(ielftype,ie,iet_["eDIFSQ"])
            vname = 'U'+str(I)
        ...
        for I in range(int(v_['0']),int(v_['NH'])+1):
            ename = 'P1'+str(I)
            [ie,ie_,_] = jtu.s2mpj_ii(ename,ie_)
            self.elftype = jtu.arrset(self.elftype,ie,'eP1')
            ielftype = jtu.arrset(ielftype,ie,iet_["eP1"])
            vname = 'U'+str(I)
            [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,None,None,None)
        ...
        for I in range(int(v_['0']),int(v_['NH-1'])+1):
            ...
            self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['ALPHAH']))
            ig = ig_['ODE1'+str(I)]
            posel = len(self.grelt[ig])
            self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['P1'+str(I)])
            nlc = jnp.union1d(nlc,jnp.array([ig]))
            self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['H/2']))
            posel = len(self.grelt[ig])
            self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['P1'+str(int(v_['I+1']))])
            nlc = jnp.union1d(nlc,jnp.array([ig]))
            self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['H/2']))
            ig = ig_['ODE2'+str(I)]
            posel = len(self.grelt[ig])
            ...
            self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['-H/2']))
            posel = len(self.grelt[ig])
            self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['P2'+str(I)])
            nlc = jnp.union1d(nlc,jnp.array([ig]))
            self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['H/2']))
            posel = len(self.grelt[ig])
            self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['P2'+str(int(v_['I+1']))])
            nlc = jnp.union1d(nlc,jnp.array([ig]))
            self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['H/2']))
        #%%%%%%%%%%%%%%%%%% OBJECT BOUNDS %%%%%%%%%%%%%%%%%
#    Solution
...
# **********************
    ...
    def eP1(self, nargout,*args):
        ...
        U_ = jtu.np_like_set(U_, jnp.array([1,2]), U_[1,2]+10)
        IV_ = jtu.np_like_set(IV_, 0, U_[0:1,:].dot(EV_))
