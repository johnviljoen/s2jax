...
# IE NPTS                5000           $-PARAMETER n = 10003
        ...
        if('Z1' in ix_):
            ...
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['Z1']),float(1.0)))
        if('Z2' in ix_):
            self.x0 = jtu.np_like_set(self.x0, ix_['Z2'], float(0.0))
        else:
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['Z2']),float(0.0)))
        if('Z3' in ix_):
            self.x0 = jtu.np_like_set(self.x0, ix_['Z3'], float(1.0))
        else:
            self.y0  = (
                  jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['Z3']),float(1.0)))
        for I in range(int(v_['1']),int(v_['NPTS'])+1):
            if('X'+str(I) in ix_):
        ...
        for I in range(int(v_['TDPulo']),int(v_['TDPuhi'])+1):
            if('X'+str(I) in ix_):
                ...
                self.y0  = (
                      jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X'+str(I)]),float(1.8)))
            if('Y'+str(I) in ix_):
                self.x0 = jtu.np_like_set(self.x0, ix_['Y'+str(I)], float(1.0))
            else:
                self.y0  = (
                      jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['Y'+str(I)]),float(1.0)))
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
