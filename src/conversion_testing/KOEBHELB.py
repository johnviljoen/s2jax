...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        self.xlower = jtu.np_like_set(self.xlower, ix_['A'], -float('Inf'))
        self.xupper = jtu.np_like_set(self.xupper, ix_['A'], +float('Inf'))
        self.xlower = jtu.np_like_set(self.xlower, ix_['B'], 0.001)
        #%%%%%%%%%%%%%%%%%%% START POINT %%%%%%%%%%%%%%%%%%
        ...
        iet_  = {}
        elftv = []
        [it,iet_,_] = jtu.s2mpj_ii( 'eKHE', iet_)
        elftv = jtu.loaset(elftv,it,0,'VN')
        elftv = jtu.loaset(elftv,it,1,'VA')
        elftv = jtu.loaset(elftv,it,2,'VB')
        elftp = []
        elftp = jtu.loaset(elftp,it,0,'XX')
        #%%%%%%%%%%%%%%%%%% ELEMENT USES %%%%%%%%%%%%%%%%%%
        ie_ = {}
        ...
        for I in range(int(v_['1']),int(v_['M'])+1):
            ...
            if newelt:
                self.elftype = jtu.arrset(self.elftype,ie,'eKHE')
                ielftype = jtu.arrset(ielftype,ie,iet_['eKHE'])
            vname = 'N'
            [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,None,None,None)
            posev = jnp.where(elftv[ielftype[ie]]=='VN')[0]
            ...
            self.elvar = jtu.loaset(self.elvar,ie,posev[0],iv)
            posep = jnp.where(elftp[ielftype[ie]]=='XX')[0]
...
# **********************
    ...
    def eKHE(self, nargout,*args):
        ...
        EV_  = args[0]
        iel_ = args[1]
        T = jnp.exp(-EV_[1]/self.elpar[iel_][0]-self.elpar[iel_][0]/EV_[2])
        ...
        if nargout>1:
            ...
            g_ = jtu.np_like_set(g_, 1, EV_[0]*T*M1OX)
            g_ = jtu.np_like_set(g_, 2, EV_[0]*T*XOB2)
            if nargout>2:
                H_ = jnp.zeros((3,3))
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), T*M1OX)
                H_ = jtu.np_like_set(H_, jnp.array([1,0]), H_[0,1])
                H_ = jtu.np_like_set(H_, jnp.array([0,2]), T*XOB2)
                H_ = jtu.np_like_set(H_, jnp.array([2,0]), H_[0,2])
                H_ = jtu.np_like_set(H_, jnp.array([1,1]), EV_[0]*T*M1OX*M1OX)
                H_ = jtu.np_like_set(H_, jnp.array([1,2]), EV_[0]*T*M1OX*XOB2)
                H_ = jtu.np_like_set(H_, jnp.array([2,1]), H_[1,2])
                H_ = jtu.np_like_set(H_, jnp.array([2,2]), EV_[0]*T*(XOB2*XOB2+M2XOB3))
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
