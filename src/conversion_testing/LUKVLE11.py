...
# IE N                   99998          $-PARAMETER
        ...
        v_['5'] = 5
        v_['N-2'] = -2+v_['N']
        v_['(N-2)/3'] = int(jnp.fix(v_['N-2']/v_['3']))
        v_['NC'] = v_['2']*v_['(N-2)/3']
        #%%%%%%%%%%%%%%%%%%%  VARIABLES %%%%%%%%%%%%%%%%%%%%
        ...
        for I in range(int(v_['1']),int(v_['(N-2)/3'])+1):
            self.gconst = jtu.arrset(self.gconst,ig_['OBJ2'+str(I)],float(1.0))
        ...
        for K in range(int(v_['1']),int(v_['NC'])+1,int(v_['2'])):
            ...
            self.gconst = jtu.arrset(self.gconst,ig_['C'+str(int(v_['K+1']))],float(2.0))
        #%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        for K in range(int(v_['1']),int(v_['NC'])+1,int(v_['2'])):
            ...
            self.elftype = jtu.arrset(self.elftype,ie,'eC21')
            ielftype = jtu.arrset(ielftype,ie,iet_["eC21"])
            vname = 'X'+str(K)
        ...
        for K in range(int(v_['1']),int(v_['NC'])+1,int(v_['2'])):
            ...
            self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['EA'+str(K)])
            nlc = jnp.union1d(nlc,jnp.array([ig]))
            self.grelw = jtu.loaset(self.grelw,ig,posel,1.)
            posel = posel+1
            self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['EB'+str(K)])
            self.grelw = jtu.loaset(self.grelw,ig,posel, 1.)
...
# **********************
    ...
    def eS(self, nargout,*args):
        ...
        if nargout>1:
            ...
            if nargout>2:
                H_ = jnp.zeros((1,1))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), -SIND)
