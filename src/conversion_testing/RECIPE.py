...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        self.gconst = jtu.arrset(self.gconst,ig_['G1'],float(5.0))
        #%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        [it,iet_,_] = jtu.s2mpj_ii( 'eXOVERU', iet_)
        elftv = jtu.loaset(elftv,it,0,'X')
        ...
        ename = 'E2'
        [ie,ie_,_] = jtu.s2mpj_ii(ename,ie_)
        self.elftype = jtu.arrset(self.elftype,ie,'eXOVERU')
        ielftype = jtu.arrset(ielftype,ie,iet_["eXOVERU"])
        vname = 'X3'
        [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,None,None,None)
        posev = jnp.where(elftv[ielftype[ie]]=='X')[0]
        self.elvar = jtu.loaset(self.elvar,ie,posev[0],iv)
    ...
    def eXOVERU(self, nargout,*args):
        ...
        if nargout>1:
            ...
            g_ = jtu.np_like_set(g_, 1, -IV_[0]/U2)
            g_ =  U_.T.dot(g_)
            if nargout>2:
                H_ = jnp.zeros((2,2))
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), -1.0/U2)
                H_ = jtu.np_like_set(H_, jnp.array([1,0]), H_[0,1])
                H_ = jtu.np_like_set(H_, jnp.array([1,1]), 2.0*IV_[0]/(U2*IV_[1]))
