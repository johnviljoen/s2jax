...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        iet_  = {}
        elftv = []
        [it,iet_,_] = jtu.s2mpj_ii( 'eWCUBE', iet_)
        ...
        for I in range(int(v_['1']),int(v_['N'])+1):
            ...
            if newelt:
                self.elftype = jtu.arrset(self.elftype,ie,'eWCUBE')
                ielftype = jtu.arrset(ielftype,ie,iet_['eWCUBE'])
            vname = 'X'+str(I)
            [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,None,None,None)
            posev = jnp.where(elftv[ielftype[ie]]=='V')[0]
            self.elvar = jtu.loaset(self.elvar,ie,posev[0],iv)
            posep = jnp.where(elftp[ielftype[ie]]=='B')[0]
            self.elpar = jtu.loaset(self.elpar,ie,posep[0],float(v_['IH+1']))
        #%%%%%%%%%%%%%%%%%%%%% GRFTYPE %%%%%%%%%%%%%%%%%%%%
        igt_ = {}
        ...
        for I in range(int(v_['1']),int(v_['N'])+1):
            ...
            self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['HALFH2']))
        #%%%%%%%%%%%%%%%%%% OBJECT BOUNDS %%%%%%%%%%%%%%%%%
#    Solution
# LO SOLTN                0.0
