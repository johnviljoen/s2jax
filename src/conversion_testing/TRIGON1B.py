...
# IE N                   1000           $-PARAMETER
        ...
        for I in range(int(v_['1']),int(v_['M'])+1):
            ...
            self.gconst = jtu.arrset(self.gconst,ig_['F'+str(I)],float(v_['RN+I']))
        #%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        self.xlower = jnp.full((self.n,1),0.0)
        self.xupper = jnp.full((self.n,1),3.141592653)
        #%%%%%%%%%%%%%%%%%%% START POINT %%%%%%%%%%%%%%%%%%
        self.x0 = jnp.zeros((self.n,1))
        self.x0 = jtu.np_like_set(self.x0, ix_['X1'], float(0.1))
        for I in range(int(v_['2']),int(v_['N'])+1):
            self.x0 = jtu.np_like_set(self.x0, ix_['X'+str(I)], float(0.1))
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
        ...
        for I in range(int(v_['1']),int(v_['N'])+1):
            ...
            vname = 'X'+str(I)
            [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,float(0.0),float(3.141592653),None)
        ...
        for I in range(int(v_['1']),int(v_['M'])+1):
            ...
            self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['RI+1']))
            posel = len(self.grelt[ig])
            self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['S'+str(I)])
            self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['RI']))
