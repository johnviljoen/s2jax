...
# IE N                   99999          $-PARAMETER
        ...
        for K in range(int(v_['1']),int(v_['N/2'])+1):
            ...
            irA  = jtu.append(irA,[ig])
            icA  = jtu.append(icA,[ix_['X'+str(int(v_['2K']))]])
        ...
        for I in range(int(v_['1']),int(v_['N'])+1):
            self.gconst = jtu.arrset(self.gconst,ig_['OBJ'+str(I)],float(-1.0))
        for K in range(int(v_['1']),int(v_['N/2'])+1):
            self.gconst = jtu.arrset(self.gconst,ig_['C'+str(K)],float(3.0))
        #%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        elftv = jtu.loaset(elftv,it,0,'VM')
        elftv = jtu.loaset(elftv,it,1,'VP')
        ...
        for K in range(int(v_['1']),int(v_['N/2'])+1):
            ...
            [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,None,None,None)
            posev = jnp.where(elftv[ielftype[ie]]=='VM')[0]
            self.elvar = jtu.loaset(self.elvar,ie,posev[0],iv)
            ...
            vname = 'X'+str(int(v_['2K+1']))
            [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,None,None,None)
            posev = jnp.where(elftv[ielftype[ie]]=='VP')[0]
            self.elvar = jtu.loaset(self.elvar,ie,posev[0],iv)
        #%%%%%%%%%%%%%%%%%%%%% GRFTYPE %%%%%%%%%%%%%%%%%%%%
        igt_ = {}
        [it,igt_,_] = jtu.s2mpj_ii('gL7d3',igt_)
        #%%%%%%%%%%%%%%%%%%% GROUP USES %%%%%%%%%%%%%%%%%%%
        self.grelt   = []
        ...
        for I in range(int(v_['1']),int(v_['N'])+1):
            ...
            v_['I-5'] = -5+I
            ig = ig_['OBJ'+str(I)]
            self.grftype = jtu.arrset(self.grftype,ig,'gL7d3')
        ...
        for K in range(int(v_['1']),int(v_['N/2'])+1):
            ig = ig_['C'+str(K)]
            posel = len(self.grelt[ig])
            self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['P'+str(K)])
            nlc = jnp.union1d(nlc,jnp.array([ig]))
            self.grelw = jtu.loaset(self.grelw,ig,posel,float(-1.0))
        #%%%%%%%%%%%%%%%%%% OBJECT BOUNDS %%%%%%%%%%%%%%%%%
        self.objlower = 0.0
...
# **********************
    ...
    def eXEXP(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, EXPW)
            g_ = jtu.np_like_set(g_, 1, UEXPW)
            g_ =  U_.T.dot(g_)
            if nargout>2:
                H_ = jnp.zeros((2,2))
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), EXPW)
                H_ = jtu.np_like_set(H_, jnp.array([1,0]), H_[0,1])
                H_ = jtu.np_like_set(H_, jnp.array([1,1]), UEXPW)
    ...
    def gL7d3(self,nargout,*args):
        ...
        GVAR_ = args[0]
        igr_  = args[1]
        Z = jnp.absolute(GVAR_)
        f_= Z**(7.0/3.0)
        if nargout>1:
            g_ = 7.0*jnp.sign(GVAR_)*(Z**(4.0/3.0))/3.0
            if nargout>2:
                H_ = jnp.zeros((1,1))
