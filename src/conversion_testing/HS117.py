...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        for J in range(int(v_['1']),int(v_['M'])+1):
            ...
            valA = jtu.append(valA,float(v_['-BJ']))
        for J in range(int(v_['1']),int(v_['N'])+1):
            for K in range(int(v_['1']),int(v_['N'])+1):
                ...
                irA  = jtu.append(irA,[ig])
                icA  = jtu.append(icA,[ix_['X'+str(int(v_['M+K']))]])
                valA = jtu.append(valA,float(v_['2CKJ']))
            for K in range(int(v_['1']),int(v_['M'])+1):
                ...
                valA = jtu.append(valA,float(v_['-AKJ']))
        #%%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
        self.n   = len(ix_)
        ngrp   = len(ig_)
        ...
        for J in range(int(v_['1']),int(v_['N'])+1):
            ...
            self.gconst = jtu.arrset(self.gconst,ig_['C'+str(J)],float(v_['-EJ']))
        #%%%%%%%%%%%%%%%%%% START POINT %%%%%%%%%%%%%%%%%%
        self.x0 = jnp.full((self.n,1),float(0.001))
        self.y0 = jnp.full((self.m,1),float(0.001))
        if('X7' in ix_):
            self.x0 = jtu.np_like_set(self.x0, ix_['X7'], float(60.0))
        else:
            self.y0  = (                   jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X7']),float(60.0)))
        pass
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
        ...
        for J in range(int(v_['1']),int(v_['N'])+1):
            ...
            [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,None,None,float(0.001))
        ...
        for J in range(int(v_['1']),int(v_['N'])+1):
            ...
            self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['2D'+str(J)])
            nlc = jnp.union1d(nlc,jnp.array([ig]))
            self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['2DJ']))
            v_['3DJ'] = 3.0*v_['D'+str(J)]
            ig = ig_['C'+str(J)]
            posel = len(self.grelt[ig])
            self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['3D'+str(J)])
            nlc = jnp.union1d(nlc,jnp.array([ig]))
            self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['3DJ']))
            for K in range(int(v_['1']),int(v_['N'])+1):
                ig = ig_['OBJ']
                posel = len(self.grelt[ig])
                self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['C'+str(K)+','+str(J)])
                nlc = jnp.union1d(nlc,jnp.array([ig]))
                self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['C'+str(K)+','+str(J)]))
        #%%%%%%%%%%%%%%%%%% OBJECT BOUNDS %%%%%%%%%%%%%%%%%
#    Solution
    ...
    def eCUBE(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, 3.0e+0*EV_[0]*EV_[0])
            if nargout>2:
                H_ = jnp.zeros((1,1))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), 6.0e+0*EV_[0])
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
