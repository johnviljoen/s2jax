...
# IE N                   10000          $-PARAMETER
        ...
        gtype        = jnp.array([])
        [ig,ig_,_] = jtu.s2mpj_ii('SQ'+str(int(v_['1'])),ig_)
        ...
        elftp = []
        elftp = jtu.loaset(elftp,it,0,'GAMMA')
        #%%%%%%%%%%%%%%%%%% ELEMENT USES %%%%%%%%%%%%%%%%%%
        ie_ = {}
        ...
        for I in range(int(v_['2']),int(v_['N'])+1):
            ...
            posep = jnp.where(elftp[ielftype[ie]]=='GAMMA')[0]
    ...
    def eETYPE(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, -self.elpar[iel_][0]*EV_[0]**(IGAMMA-1))
            if nargout>2:
                H_ = jnp.zeros((1,1))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), -self.elpar[iel_][0]*(self.elpar[iel_][0]-1.0)*EV_[0]**(IGAMMA-2))
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
