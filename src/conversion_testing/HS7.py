...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        iet_  = {}
        elftv = []
        [it,iet_,_] = jtu.s2mpj_ii( 'eSQp1', iet_)
        elftv = jtu.loaset(elftv,it,0,'V1')
        [it,iet_,_] = jtu.s2mpj_ii( 'eSQ', iet_)
        elftv = jtu.loaset(elftv,it,0,'V1')
        [it,iet_,_] = jtu.s2mpj_ii( 'eLN', iet_)
        elftv = jtu.loaset(elftv,it,0,'V1')
        #%%%%%%%%%%%%%%%%%% ELEMENT USES %%%%%%%%%%%%%%%%%%
        ie_ = {}
        ...
        self.elftype = jtu.arrset(self.elftype,ie,'eSQp1')
        ielftype = jtu.arrset(ielftype,ie,iet_["eSQp1"])
        vname = 'X1'
        ...
        ename = 'E3'
        [ie,ie_,_] = jtu.s2mpj_ii(ename,ie_)
        self.elftype = jtu.arrset(self.elftype,ie,'eLN')
        ielftype = jtu.arrset(ielftype,ie,iet_["eLN"])
        vname = 'X1'
        [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,None,None,None)
        posev = jnp.where(elftv[ielftype[ie]]=='V1')[0]
        self.elvar = jtu.loaset(self.elvar,ie,posev[0],iv)
...
# **********************
    ...
    @staticmethod
    def eSQp1(self, nargout,*args):

        import jax.numpy as jnp
        EV_  = args[0]
        iel_ = args[1]
        XXP1 = 1.0+EV_[0]*EV_[0]
        f_   = XXP1**2
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, 4.0*EV_[0]*XXP1)
            if nargout>2:
                H_ = jnp.zeros((1,1))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), 4.0*(1.0+3.0*EV_[0]*EV_[0]))
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
    ...
    def eLN(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, 2.0*EV_[0]/XXP1)
            if nargout>2:
                H_ = jnp.zeros((1,1))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), 2.0/XXP1-4.0*EV_[0]*EV_[0]/(XXP1)**2)
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
