...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        if('T0' in ix_):
            ...
            self.y0  = (                   jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['T0']),float(107.47)))
        if('T1' in ix_):
            self.x0 = jtu.np_like_set(self.x0, ix_['T1'], float(102.4))
        else:
            self.y0  = (                   jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['T1']),float(102.4)))
        if('T2' in ix_):
            self.x0 = jtu.np_like_set(self.x0, ix_['T2'], float(97.44))
        else:
            self.y0  = (                   jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['T2']),float(97.44)))
        if('T3' in ix_):
            self.x0 = jtu.np_like_set(self.x0, ix_['T3'], float(96.3))
        else:
            self.y0  = (                   jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['T3']),float(96.3)))
        if('T4' in ix_):
            self.x0 = jtu.np_like_set(self.x0, ix_['T4'], float(93.99))
        else:
            self.y0  = (                   jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['T4']),float(93.99)))
        if('T5' in ix_):
            self.x0 = jtu.np_like_set(self.x0, ix_['T5'], float(89.72))
        else:
            self.y0  = (                   jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['T5']),float(89.72)))
        if('T6' in ix_):
            self.x0 = jtu.np_like_set(self.x0, ix_['T6'], float(83.71))
        else:
            self.y0  = (                   jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['T6']),float(83.71)))
        if('T7' in ix_):
            self.x0 = jtu.np_like_set(self.x0, ix_['T7'], float(78.31))
        else:
            self.y0  = (                   jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['T7']),float(78.31)))
        if('V0' in ix_):
