    ...
    def __init__(self, *args): 
        ...
        for I in range(int(v_['2']),int(v_['P'])+1):
            ...
            for J in range(int(v_['2']),int(v_['P'])+1):
                ...
                irA  = jtu.append(irA,[ig])
                icA  = jtu.append(icA,[ix_['X'+str(I)+','+str(int(v_['J-1']))]])
                valA = jtu.append(valA,float(-1.0))
                ...
                valA = jtu.append(valA,float(1.0))
                irA  = jtu.append(irA,[ig])
                icA  = jtu.append(icA,[ix_['X'+str(int(v_['I-1']))+','+str(J)]])
                valA = jtu.append(valA,float(-1.0))
                [ig,ig_,_] = jtu.s2mpj_ii('C'+str(I)+','+str(J),ig_)
                gtype = jtu.arrset(gtype,ig,'<>')
                self.gscale = jtu.arrset(self.gscale,ig,float(v_['1/HP2']))
                irA  = jtu.append(irA,[ig])
                icA  = jtu.append(icA,[ix_['X'+str(I)+','+str(J)]])
                valA = jtu.append(valA,float(1.0))
                irA  = jtu.append(irA,[ig])
        ...
        gtype = jtu.arrset(gtype,ig,'<>')
        irA  = jtu.append(irA,[ig])
        icA  = jtu.append(icA,[ix_['X'+str(int(v_['P']))+','+str(int(v_['P']))]])
        valA = jtu.append(valA,float(v_['WGHT']))
        #%%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
        self.n   = len(ix_)
        ngrp   = len(ig_)
