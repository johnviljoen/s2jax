...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        self.xlower = jtu.np_like_set(self.xlower, ix_['X1'], -0.5)
        self.xupper = jtu.np_like_set(self.xupper, ix_['X1'], 0.5)
        self.xlower = jtu.arrset(self.xlower,ix_['X2'],-float('Inf'))
    ...
    def eSQSQ(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, -4.0*TM1SQ*EV_[0])
            g_ = jtu.np_like_set(g_, 1, 2.0*TM1SQ)
            if nargout>2:
                H_ = jnp.zeros((2,2))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), 12.0*EV_[0]*EV_[0]-4.0*EV_[1])
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), -4.0*EV_[0])
    ...
    def eSQm1(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, -2.0*WMV1)
            if nargout>2:
                H_ = jnp.zeros((1,1))
