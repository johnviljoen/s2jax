    ...
    def __init__(self, *args): 
        ...
        for I in range(int(v_['1']),int(v_['NP'])+1):
            ...
            if('X'+str(I) in ix_):
                ...
                self.y0  = (
                      jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['X'+str(I)]),float(v_['X'])))
            if('Y'+str(I) in ix_):
                self.x0 = jtu.np_like_set(self.x0, ix_['Y'+str(I)], float(v_['Y']))
            else:
                self.y0  = (
                      jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['Y'+str(I)]),float(v_['Y'])))
            if('Z'+str(I) in ix_):
                self.x0 = jtu.np_like_set(self.x0, ix_['Z'+str(I)], float(v_['Z']))
            else:
                self.y0  = (
                      jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['Z'+str(I)]),float(v_['Z'])))
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
