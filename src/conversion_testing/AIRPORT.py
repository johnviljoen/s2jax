...
#    disjoint. The problem is to jtu.find one point  (xi, yi) on  each ball, i=1,..,m,
    ...
    def __init__(self, *args): 
        ...
        for I in range(int(v_['1']),int(v_['N-1'])+1):
            ...
            for J in range(int(v_['I+1']),int(v_['N'])+1):
                [ig,ig_,_] = jtu.s2mpj_ii('OBJ1'+str(I)+','+str(J),ig_)
                ...
                valA = jtu.append(valA,float(-1.0))
                [ig,ig_,_] = jtu.s2mpj_ii('OBJ2'+str(I)+','+str(J),ig_)
                gtype = jtu.arrset(gtype,ig,'<>')
                irA  = jtu.append(irA,[ig])
                icA  = jtu.append(icA,[ix_['Y'+str(I)]])
                valA = jtu.append(valA,float(1.0))
                irA  = jtu.append(irA,[ig])
                icA  = jtu.append(icA,[ix_['Y'+str(J)]])
                valA = jtu.append(valA,float(-1.0))
        for I in range(int(v_['1']),int(v_['N'])+1):
        ...
        for I in range(int(v_['1']),int(v_['N'])+1):
            self.gconst = jtu.arrset(self.gconst,ig_['CONS'+str(I)],float(v_['R'+str(I)]))
        #%%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        self.xlower = jnp.zeros((self.n,1))
        self.xupper = jnp.full((self.n,1),float('inf'))
        for I in range(int(v_['1']),int(v_['N'])+1):
            self.xlower = jtu.np_like_set(self.xlower, ix_['X'+str(I)], -10)
            self.xupper = jtu.np_like_set(self.xupper, ix_['X'+str(I)], 10)
            self.xlower = jtu.np_like_set(self.xlower, ix_['Y'+str(I)], -10)
            self.xupper = jtu.np_like_set(self.xupper, ix_['Y'+str(I)], 10)
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
        ...
        for I in range(int(v_['1']),int(v_['N'])+1):
            ...
            self.elpar = jtu.loaset(self.elpar,ie,posep[0],float(v_['CX'+str(I)]))
        for I in range(int(v_['1']),int(v_['N'])+1):
            ename = 'B'+str(I)
            [ie,ie_,_] = jtu.s2mpj_ii(ename,ie_)
            self.elftype = jtu.arrset(self.elftype,ie,'eDIFSQR')
            ...
            self.elvar = jtu.loaset(self.elvar,ie,posev[0],iv)
            posep = jnp.where(elftp[ielftype[ie]]=='W')[0]
            self.elpar = jtu.loaset(self.elpar,ie,posep[0],float(v_['CY'+str(I)]))
        #%%%%%%%%%%%%%%%%%%%%% GRFTYPE %%%%%%%%%%%%%%%%%%%%
        igt_ = {}
