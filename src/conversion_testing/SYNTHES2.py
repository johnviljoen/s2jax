...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        irA  = jtu.append(irA,[ig])
        icA  = jtu.append(icA,[ix_['X3']])
        valA = jtu.append(valA,float(1.25))
        irA  = jtu.append(irA,[ig])
        icA  = jtu.append(icA,[ix_['Y3']])
        valA = jtu.append(valA,float(-10.0))
        ...
        self.gconst = jtu.arrset(self.gconst,ig_['OBJ'],float(-140.0))
        self.gconst = jtu.arrset(self.gconst,ig_['N2'],float(1.0))
        self.gconst = jtu.arrset(self.gconst,ig_['N3'],float(1.0))
        self.gconst = jtu.arrset(self.gconst,ig_['L10'],float(1.0))
        self.gconst = jtu.arrset(self.gconst,ig_['L11'],float(1.0))
        #%%%%%%%%%%%%%%%%%%%%  RANGES %%%%%%%%%%%%%%%%%%%%%%
        grange = jnp.full((ngrp,1),None)
        grange = jtu.np_like_set(grange, legrps, jnp.full((self.nle,1),float('inf')))
        grange = jtu.arrset(grange,ig_['L11'],float(-1.0))
        #%%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        self.xupper = jtu.np_like_set(self.xupper, ix_['X6'], 3.0)
        for I in range(int(v_['1']),int(v_['5'])+1):
        ...
        self.grelw = jtu.loaset(self.grelw,ig,posel,float(1.0))
        posel = len(self.grelt[ig])
        self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['LOGX4X5'])
        nlc = jnp.union1d(nlc,jnp.array([ig]))
        self.grelw = jtu.loaset(self.grelw,ig,posel,float(-60.0))
        ig = ig_['N1']
        posel = len(self.grelt[ig])
        self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['LOGX4X5'])
        nlc = jnp.union1d(nlc,jnp.array([ig]))
