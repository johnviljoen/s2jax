...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        gtype        = jnp.array([])
        [ig,ig_,_] = jtu.s2mpj_ii('GC'+str(int(v_['0'])),ig_)
        ...
        for I in range(int(v_['1']),int(v_['N'])+1):
            ...
            gtype = jtu.arrset(gtype,ig,'<>')
            [ig,ig_,_] = jtu.s2mpj_ii('GC'+str(I),ig_)
            gtype = jtu.arrset(gtype,ig,'<>')
        #%%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
        self.n   = len(ix_)
        ngrp   = len(ig_)
        ...
        for I in range(int(v_['1']),int(v_['N'])+1):
            ...
            v_['10X0'] = v_['NUM']*v_['H']
            v_['VAL'] = 0.1*v_['10X0']
            self.x0 = jtu.np_like_set(self.x0, ix_['X'+str(I)], float(v_['VAL']))
        ...
        [it,iet_,_] = jtu.s2mpj_ii( 'eBRA', iet_)
        elftv = jtu.loaset(elftv,it,0,'X')
        elftv = jtu.loaset(elftv,it,1,'Y')
        #%%%%%%%%%%%%%%%%%% ELEMENT USES %%%%%%%%%%%%%%%%%%
        ie_ = {}
        ...
        self.elftype = jtu.arrset(self.elftype,ie,'eBRA')
        ielftype = jtu.arrset(ielftype,ie,iet_["eBRA"])
        ename = 'C'+str(int(v_['0']))
        [ie,ie_,_] = jtu.s2mpj_ii(ename,ie_)
        ...
        self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['C'+str(int(v_['0']))])
        self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['2LH']))
        for I in range(int(v_['1']),int(v_['N'])+1):
            ig = ig_['GA'+str(I)]
            posel = len(self.grelt[ig])
            self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['A'+str(I)])
            self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['2/H']))
            ig = ig_['GB'+str(I)]
            posel = len(self.grelt[ig])
    ...
    def eBRA(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, -EX/D+DEXD2)
            g_ = jtu.np_like_set(g_, 1, EY/D-DEXD2)
            if nargout>2:
                H_ = jnp.zeros((2,2))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), -EX/D-2.0*(EX/D2-DEXD3))
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), (EX+EY)/D2-2.0*DEXD3)
                H_ = jtu.np_like_set(H_, jnp.array([1,0]), H_[0,1])
                H_ = jtu.np_like_set(H_, jnp.array([1,1]), EY/D-2.0*(EY/D2-DEXD3))
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
