...
# IE P                   16             $-PARAMETER n = 65537
        ...
        for L in range(int(v_['1']),int(v_['N/P'])+1):
            for I in range(int(v_['1']),int(v_['P'])+1):
                ...
                for J in range(int(v_['1']),int(I)+1):
                    v_['K+L'] = v_['K']+L
                    [ig,ig_,_] = jtu.s2mpj_ii('N'+str(I)+','+str(L),ig_)
                    gtype = jtu.arrset(gtype,ig,'<>')
                    irA  = jtu.append(irA,[ig])
                    icA  = jtu.append(icA,[ix_['X'+str(int(v_['K+L']))]])
                    valA = jtu.append(valA,float(1.0))
                    v_['K'] = v_['K']*v_['2']
        for L in range(int(v_['1']),int(v_['N/P'])+1,int(v_['2'])):
            for I in range(int(v_['1']),int(v_['P'])+1):
                ...
                for J in range(int(v_['1']),int(I)+1):
                    v_['K-1'] = v_['K']-v_['1']
                    v_['K+L-1'] = v_['K-1']+L
                    [ig,ig_,_] = jtu.s2mpj_ii('E'+str(I)+','+str(L),ig_)
                    gtype = jtu.arrset(gtype,ig,'==')
                    cnames = jtu.arrset(cnames,ig,'E'+str(I)+','+str(L))
                    irA  = jtu.append(irA,[ig])
                    icA  = jtu.append(icA,[ix_['X'+str(int(v_['K+L-1']))]])
                    valA = jtu.append(valA,float(1.0))
                    v_['K'] = v_['K']*v_['2']
        #%%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
        self.n   = len(ix_)
        ...
        for L in range(int(v_['1']),int(v_['N/P'])+1,int(v_['2'])):
            for I in range(int(v_['1']),int(v_['P'])+1):
                v_['RI'] = float(I)
                self.gconst = jtu.arrset(self.gconst,ig_['E'+str(I)+','+str(L)],float(v_['RI']))
        #%%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        self.xlower = jnp.zeros((self.n,1))
        self.xupper = jnp.full((self.n,1),float('inf'))
        for I in range(int(v_['0']),int(v_['N'])+1):
            self.xlower = jtu.np_like_set(self.xlower, ix_['X'+str(I)], -2.0)
            self.xupper = jtu.np_like_set(self.xupper, ix_['X'+str(I)], 2.0)
        #%%%%%%%%%%%%%%%%%% START POINT %%%%%%%%%%%%%%%%%%
        self.x0 = jnp.full((self.n,1),float(0.5))
        #%%%%%%%%%%%%%%%%%%%%% GRFTYPE %%%%%%%%%%%%%%%%%%%%
        igt_ = {}
        [it,igt_,_] = jtu.s2mpj_ii('gPSQR',igt_)
        [it,igt_,_] = jtu.s2mpj_ii('gPSQR',igt_)
        grftp = []
        ...
        for I in range(int(v_['0']),int(v_['N'])+1):
            ig = ig_['O'+str(I)]
            self.grftype = jtu.arrset(self.grftype,ig,'gPSQR')
