...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        iet_  = {}
        elftv = []
        [it,iet_,_] = jtu.s2mpj_ii( 'eE15', iet_)
        ...
        for I in range(int(v_['1']),int(v_['M'])+1):
            ...
            self.elftype = jtu.arrset(self.elftype,ie,'eE15')
            ielftype = jtu.arrset(ielftype,ie,iet_["eE15"])
            vname = 'B1'
    ...
    def eE15(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, 1.0/V2PXP)
            g_ = jtu.np_like_set(g_, 1, -EV_[0]/(EV_[2]*V2PXP1))
            g_ = jtu.np_like_set(g_, 2, EV_[0]*V2PXL/(V2PXP*EV_[2]**2))
            if nargout>2:
                H_ = jnp.zeros((3,3))
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), -1.0/(EV_[2]*V2PXP1))
                H_ = jtu.np_like_set(H_, jnp.array([1,0]), H_[0,1])
                H_ = jtu.np_like_set(H_, jnp.array([0,2]), V2PXL/(V2PXP*EV_[2]**2))
