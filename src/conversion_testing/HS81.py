...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        self.gconst = jtu.arrset(self.gconst,ig_['OBJ'],float(0.5))
        self.gconst = jtu.arrset(self.gconst,ig_['C1'],float(10.0))
        self.gconst = jtu.arrset(self.gconst,ig_['C3'],float(-1.0))
        #%%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        self.xlower = jnp.zeros((self.n,1))
        self.xupper = jnp.full((self.n,1),float('inf'))
        self.xlower = jtu.np_like_set(self.xlower, ix_['X1'], -2.3)
        self.xupper = jtu.np_like_set(self.xupper, ix_['X1'], 2.3)
        self.xlower = jtu.np_like_set(self.xlower, ix_['X2'], -2.3)
        self.xupper = jtu.np_like_set(self.xupper, ix_['X2'], 2.3)
        self.xlower = jtu.np_like_set(self.xlower, ix_['X3'], -3.2)
        self.xupper = jtu.np_like_set(self.xupper, ix_['X3'], 3.2)
        self.xlower = jtu.np_like_set(self.xlower, ix_['X4'], -3.2)
        self.xupper = jtu.np_like_set(self.xupper, ix_['X4'], 3.2)
        self.xlower = jtu.np_like_set(self.xlower, ix_['X5'], -3.2)
        self.xupper = jtu.np_like_set(self.xupper, ix_['X5'], 3.2)
        #%%%%%%%%%%%%%%%%%%% START POINT %%%%%%%%%%%%%%%%%%
        ...
        iet_  = {}
        elftv = []
        [it,iet_,_] = jtu.s2mpj_ii( 'eEXPO', iet_)
        ...
        [it,iet_,_] = jtu.s2mpj_ii( 'eX6', iet_)
        elftv = jtu.loaset(elftv,it,0,'X')
        [it,iet_,_] = jtu.s2mpj_ii( 'eX3Y3', iet_)
        elftv = jtu.loaset(elftv,it,0,'X')
        ...
        self.elftype = jtu.arrset(self.elftype,ie,'eEXPO')
        ielftype = jtu.arrset(ielftype,ie,iet_["eEXPO"])
        vname = 'X1'
        ...
        ename = 'OBJ2'
        [ie,ie_,_] = jtu.s2mpj_ii(ename,ie_)
        self.elftype = jtu.arrset(self.elftype,ie,'eX6')
        ielftype = jtu.arrset(ielftype,ie,iet_["eX6"])
        vname = 'X1'
        [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,None,None,None)
        ...
        ename = 'OBJ4'
        [ie,ie_,_] = jtu.s2mpj_ii(ename,ie_)
        self.elftype = jtu.arrset(self.elftype,ie,'eX3Y3')
        ielftype = jtu.arrset(ielftype,ie,iet_["eX3Y3"])
        vname = 'X1'
        [iv,ix_] = jtu.s2mpj_nlx(self,vname,ix_,1,None,None,None)
        posev = jnp.where(elftv[ielftype[ie]]=='X')[0]
        self.elvar = jtu.loaset(self.elvar,ie,posev[0],iv)
        ...
        posel = posel+1
        self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['OBJ2'])
        self.grelw = jtu.loaset(self.grelw,ig,posel,float(-0.5))
        posel = len(self.grelt[ig])
        self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['OBJ3'])
        nlc = jnp.union1d(nlc,jnp.array([ig]))
        self.grelw = jtu.loaset(self.grelw,ig,posel,float(-0.5))
        posel = posel+1
        self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['OBJ4'])
        self.grelw = jtu.loaset(self.grelw,ig,posel,float(-1.0))
        posel = len(self.grelt[ig])
        self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['OBJ5'])
        nlc = jnp.union1d(nlc,jnp.array([ig]))
        self.grelw = jtu.loaset(self.grelw,ig,posel,float(-1.0))
        posel = posel+1
        self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['OBJ6'])
        self.grelw = jtu.loaset(self.grelw,ig,posel,float(-1.0))
        ig = ig_['C1']
        posel = len(self.grelt[ig])
    ...
    def eEXPO(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, EX*EV_[1]*EV_[2]*EV_[3]*EV_[4])
            g_ = jtu.np_like_set(g_, 1, EX*EV_[0]*EV_[2]*EV_[3]*EV_[4])
            g_ = jtu.np_like_set(g_, 2, EX*EV_[0]*EV_[1]*EV_[3]*EV_[4])
            g_ = jtu.np_like_set(g_, 3, EX*EV_[0]*EV_[1]*EV_[2]*EV_[4])
            g_ = jtu.np_like_set(g_, 4, EX*EV_[0]*EV_[1]*EV_[2]*EV_[3])
            if nargout>2:
                H_ = jnp.zeros((5,5))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), EX*(EV_[1]*EV_[2]*EV_[3]*EV_[4])**2)
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), EX1*EV_[2]*EV_[3]*EV_[4])
                H_ = jtu.np_like_set(H_, jnp.array([1,0]), H_[0,1])
                H_ = jtu.np_like_set(H_, jnp.array([0,2]), EX1*EV_[1]*EV_[3]*EV_[4])
                H_ = jtu.np_like_set(H_, jnp.array([2,0]), H_[0,2])
                H_ = jtu.np_like_set(H_, jnp.array([0,3]), EX1*EV_[1]*EV_[2]*EV_[4])
                H_ = jtu.np_like_set(H_, jnp.array([3,0]), H_[0,3])
                H_ = jtu.np_like_set(H_, jnp.array([0,4]), EX1*EV_[1]*EV_[2]*EV_[3])
                H_ = jtu.np_like_set(H_, jnp.array([4,0]), H_[0,4])
                H_ = jtu.np_like_set(H_, jnp.array([1,1]), EX*(EV_[0]*EV_[2]*EV_[3]*EV_[4])**2)
                H_ = jtu.np_like_set(H_, jnp.array([1,2]), EX1*EV_[0]*EV_[3]*EV_[4])
                H_ = jtu.np_like_set(H_, jnp.array([2,1]), H_[1,2])
                H_ = jtu.np_like_set(H_, jnp.array([1,3]), EX1*EV_[0]*EV_[2]*EV_[4])
                H_ = jtu.np_like_set(H_, jnp.array([3,1]), H_[1,3])
                H_ = jtu.np_like_set(H_, jnp.array([1,4]), EX1*EV_[0]*EV_[2]*EV_[3])
                H_ = jtu.np_like_set(H_, jnp.array([4,1]), H_[1,4])
                H_ = jtu.np_like_set(H_, jnp.array([2,2]), EX*(EV_[0]*EV_[1]*EV_[3]*EV_[4])**2)
                H_ = jtu.np_like_set(H_, jnp.array([2,3]), EX1*EV_[0]*EV_[1]*EV_[4])
                H_ = jtu.np_like_set(H_, jnp.array([3,2]), H_[2,3])
                H_ = jtu.np_like_set(H_, jnp.array([2,4]), EX1*EV_[0]*EV_[1]*EV_[3])
                H_ = jtu.np_like_set(H_, jnp.array([4,2]), H_[2,4])
                H_ = jtu.np_like_set(H_, jnp.array([3,3]), EX*(EV_[0]*EV_[1]*EV_[2]*EV_[4])**2)
                H_ = jtu.np_like_set(H_, jnp.array([3,4]), EX1*EV_[0]*EV_[1]*EV_[2])
                H_ = jtu.np_like_set(H_, jnp.array([4,3]), H_[3,4])
                H_ = jtu.np_like_set(H_, jnp.array([4,4]), EX*(EV_[0]*EV_[1]*EV_[2]*EV_[3])**2)
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
    ...
    def eX3Y3(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, 3.0*X2*Y3)
            g_ = jtu.np_like_set(g_, 1, 3.0*X3*Y2)
            if nargout>2:
                H_ = jnp.zeros((2,2))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), 6.0*EV_[0]*Y3)
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), 9.0*X2*Y2)
                H_ = jtu.np_like_set(H_, jnp.array([1,0]), H_[0,1])
                H_ = jtu.np_like_set(H_, jnp.array([1,1]), 6.0*X3*EV_[1])
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
