...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        for I in range(int(v_['1']),int(v_['M'])+1):
            ...
            v_['T'] = 2*v_['I+1']
            v_['2+2I'] = float(v_['T'])
            self.gconst = jtu.arrset(self.gconst,ig_['G'+str(I)],float(v_['2+2I']))
        #%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        self.xlower = jnp.full((self.n,1),-float('Inf'))
        self.xupper = jnp.full((self.n,1),+float('Inf'))
        #%%%%%%%%%%%%%%%%%%% START POINT %%%%%%%%%%%%%%%%%%
        self.x0 = jnp.zeros((self.n,1))
        self.x0 = jtu.np_like_set(self.x0, ix_['X1'], float(0.3))
        self.x0 = jtu.np_like_set(self.x0, ix_['X2'], float(0.4))
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
...
# **********************
    ...
    def ePEXP(self, nargout,*args):
        ...
        EV_  = args[0]
        iel_ = args[1]
        T = jnp.exp(self.elpar[iel_][0]*EV_[0])
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, T1)
            if nargout>2:
                H_ = jnp.zeros((1,1))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), self.elpar[iel_][0]*T1)
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
