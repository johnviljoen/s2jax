...
# IE N                   1000           $-PARAMETER
        ...
        iet_  = {}
        elftv = []
        [it,iet_,_] = jtu.s2mpj_ii( 'eSV', iet_)
        ...
        for I in range(int(v_['1']),int(v_['M'])+1):
            ...
            self.elftype = jtu.arrset(self.elftype,ie,'eSV')
            ielftype = jtu.arrset(ielftype,ie,iet_["eSV"])
            vname = 'X'+str(I)
    ...
    def eSV(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, AX1*B+A*SX1)
            g_ = jtu.np_like_set(g_, 1, AX2*B+A*SX2)
            if nargout>2:
                H_ = jnp.zeros((2,2))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), AX1X1*B+AX1*SX1+AX1*SX1+A*SX1X1)
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), AX1X2*B+AX2*SX1+AX1*SX2+A*SX1X2)
                H_ = jtu.np_like_set(H_, jnp.array([1,0]), H_[0,1])
                H_ = jtu.np_like_set(H_, jnp.array([1,1]), AX2X2*B+AX2*SX2+AX2*SX2+A*SX2X2)
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
