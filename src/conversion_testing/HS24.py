...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        valA = jtu.append(valA,float(v_['C1FAC']))
        irA  = jtu.append(irA,[ig])
        ...
        irA  = jtu.append(irA,[ig])
        icA  = jtu.append(icA,[ix_['X2']])
        valA = jtu.append(valA,float(v_['RT3']))
        ...
        irA  = jtu.append(irA,[ig])
        icA  = jtu.append(icA,[ix_['X2']])
        valA = jtu.append(valA,float(v_['MRT3']))
        #%%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
        self.n   = len(ix_)
        ngrp   = len(ig_)
...
# **********************
    ...
    def eOBJ(self, nargout,*args):
        ...
        EV_  = args[0]
        iel_ = args[1]
        RT3X27 = 1.0e0/(27.0e0*jnp.sqrt(3.0e0))
        V2SQ = EV_[1]*EV_[1]
        V2CB = V2SQ*EV_[1]
        X1M3 = EV_[0]-3.0
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, 2.0*X1M3*RT3X27*V2CB)
            g_ = jtu.np_like_set(g_, 1, 3.0*X1M3M9*RT3X27*V2SQ)
            if nargout>2:
                H_ = jnp.zeros((2,2))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), 2.0*RT3X27*V2CB)
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), 6.0*X1M3*RT3X27*V2SQ)
                H_ = jtu.np_like_set(H_, jnp.array([1,0]), H_[0,1])
                H_ = jtu.np_like_set(H_, jnp.array([1,1]), 6.0*X1M3M9*RT3X27*EV_[1])
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
