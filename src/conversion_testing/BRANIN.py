...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        self.gconst = jtu.arrset(self.gconst,ig_['OBJ1'],float(v_['R']))
        self.gconst = jtu.arrset(self.gconst,ig_['OBJ2'],float(-10.0))
        #%%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        self.xlower = jnp.zeros((self.n,1))
        self.xupper = jnp.full((self.n,1),float('inf'))
        self.xlower = jtu.np_like_set(self.xlower, ix_['X1'], -5.0)
        self.xupper = jtu.np_like_set(self.xupper, ix_['X1'], 10.0)
        self.xlower = jtu.np_like_set(self.xlower, ix_['X2'], 0.0)
        self.xupper = jtu.np_like_set(self.xupper, ix_['X2'], 15.0)
        pass
        #%%%%%%%%%%%%%%%%%%% START POINT %%%%%%%%%%%%%%%%%%
        ...
        self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['-B']))
        posgp = jnp.where(grftp[igt_[self.grftype[ig]]]=='P')[0]
        self.grpar =jtu.loaset(self.grpar,ig,posgp[0],float(v_['A']))
        ig = ig_['OBJ2']
        posel = len(self.grelt[ig])
        self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['E2'])
        self.grelw = jtu.loaset(self.grelw,ig,posel,float(v_['S1MT']))
        #%%%%%%%%%%%%%%%%%% OBJECT BOUNDS %%%%%%%%%%%%%%%%%
#    Solution
# LO SOLTN               -1.031628
...
# **********************
    ...
    def eCOS(self, nargout,*args):
        ...
        if nargout>1:
            ...
            if nargout>2:
                H_ = jnp.zeros((1,1))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), -jnp.cos(EV_[0]))
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
