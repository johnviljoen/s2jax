...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        self.x0 = jtu.np_like_set(self.x0, ix_['B2'], float(-0.1))
        self.x0 = jtu.np_like_set(self.x0, ix_['B3'], float(0.003))
        self.x0 = jtu.np_like_set(self.x0, ix_['B4'], float(-0.001))
        self.x0 = jtu.np_like_set(self.x0, ix_['B5'], float(0.00001))
        pass
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
        [it,iet_,_] = jtu.s2mpj_ii( 'eE18', iet_)
        ...
        for I in range(int(v_['1']),int(v_['M'])+1):
            ...
            self.elftype = jtu.arrset(self.elftype,ie,'eE18')
            ielftype = jtu.arrset(ielftype,ie,iet_["eE18"])
            vname = 'B1'
    ...
    def eE18(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, 1/B)
            g_ = jtu.np_like_set(g_, 1, self.elpar[iel_][0]/B)
            g_ = jtu.np_like_set(g_, 2, X2/B)
            g_ = jtu.np_like_set(g_, 3, -T*self.elpar[iel_][0]/B2)
            g_ = jtu.np_like_set(g_, 4, -T*X2/B2)
            if nargout>2:
                H_ = jnp.zeros((5,5))
                H_ = jtu.np_like_set(H_, jnp.array([0,3]), -self.elpar[iel_][0]/B2)
                H_ = jtu.np_like_set(H_, jnp.array([3,0]), H_[0,3])
                H_ = jtu.np_like_set(H_, jnp.array([0,4]), -X2/B2)
                H_ = jtu.np_like_set(H_, jnp.array([4,0]), H_[0,4])
                H_ = jtu.np_like_set(H_, jnp.array([1,3]), -X2/B2)
                H_ = jtu.np_like_set(H_, jnp.array([3,1]), H_[1,3])
                H_ = jtu.np_like_set(H_, jnp.array([1,4]), -X3/B2)
                H_ = jtu.np_like_set(H_, jnp.array([4,1]), H_[1,4])
                H_ = jtu.np_like_set(H_, jnp.array([2,3]), -X3/B2)
                H_ = jtu.np_like_set(H_, jnp.array([3,2]), H_[2,3])
                H_ = jtu.np_like_set(H_, jnp.array([2,4]), -X4/B2)
                H_ = jtu.np_like_set(H_, jnp.array([4,2]), H_[2,4])
                H_ = jtu.np_like_set(H_, jnp.array([3,3]), 2.0*T*X2/B3)
                H_ = jtu.np_like_set(H_, jnp.array([3,4]), 2.0*T*X3/B3)
                H_ = jtu.np_like_set(H_, jnp.array([4,3]), H_[3,4])
                H_ = jtu.np_like_set(H_, jnp.array([4,4]), 2.0*T*X4/B3)
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
