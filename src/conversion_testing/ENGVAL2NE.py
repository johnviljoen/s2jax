...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        self.gconst = jtu.arrset(self.gconst,ig_['G3'],float(1.0))
        self.gconst = jtu.arrset(self.gconst,ig_['G4'],float(-1.0))
        self.gconst = jtu.arrset(self.gconst,ig_['G5'],float(36.0))
        #%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
    ...
    def eELT(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, 3.0*EV_[0]**2-2.0*TWOW)
            g_ = jtu.np_like_set(g_, 1, 10.0*TWOW)
            if nargout>2:
                H_ = jnp.zeros((2,2))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), 6.0*EV_[0]+2.0)
                H_ = jtu.np_like_set(H_, jnp.array([0,1]), -10.0)
                H_ = jtu.np_like_set(H_, jnp.array([1,0]), H_[0,1])
                H_ = jtu.np_like_set(H_, jnp.array([1,1]), 50.0)
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
