...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        if('B' in ix_):
            self.x0 = jtu.np_like_set(self.x0, ix_['B'], float(5.2))
        else:
            self.y0  = (                   jtu.arrset(self.y0,findfirst(self.congrps,lambda x:x==ig_['B']),float(5.2)))
        pass
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
        ...
        for I in range(int(v_['1']),int(v_['M'])+1):
            ...
            self.grelt = jtu.loaset(self.grelt,ig,posel,ie_['G'+str(I)])
    ...
    def eSINE(self, nargout,*args):
        ...
        if nargout>1:
            try:
                ...
                dim = len(EV_)
            g_ = jnp.zeros(dim)
            g_ = jtu.np_like_set(g_, 0, self.elpar[iel_][0]*C)
            if nargout>2:
                H_ = jnp.zeros((1,1))
                H_ = jtu.np_like_set(H_, jnp.array([0,0]), -self.elpar[iel_][0]*self.elpar[iel_][0]*S)
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
