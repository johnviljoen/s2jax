...
# IE N                   50000          $-PARAMETER 
        ...
        for I in range(int(v_['1']),int(v_['N'])+1):
            [ig,ig_,_] = jtu.s2mpj_ii('CX',ig_)
            gtype = jtu.arrset(gtype,ig,'==')
            cnames = jtu.arrset(cnames,ig,'CX')
            irA  = jtu.append(irA,[ig])
            icA  = jtu.append(icA,[ix_['X'+str(I)]])
            valA = jtu.append(valA,float(1.0))
            irA  = jtu.append(irA,[ig])
        ...
        for I in range(int(v_['1']),int(v_['N'])+1):
            v_['RI'] = float(I)
            self.gconst = jtu.arrset(self.gconst,ig_['E'+str(I)],float(v_['RI']))
        self.gconst = jtu.arrset(self.gconst,ig_['CX'],float(v_['RN/2']))
        #%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
